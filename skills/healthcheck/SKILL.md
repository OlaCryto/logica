---
name: healthcheck
description: Host security hardening and risk-tolerance configuration for Logica deployments. Use when a user asks for security audits, firewall/SSH/update hardening, risk posture, exposure review, Logica cron scheduling for periodic checks, or version status checks on a machine running Logica (laptop, workstation, Pi, VPS).
---

# Logica Host Hardening

## Overview

Assess and harden the host running Logica, then align it to a user-defined risk tolerance without breaking access. Use Logica security tooling as a first-class signal, but treat OS hardening as a separate, explicit set of steps.
...
- Never claim Logica changes the host firewall, SSH, or OS updates; it does not.
...
5. Logica gateway status and bind address.
...
9. OS automatic security updates status.
   Note: these are not blocking items, but are highly recommended, especially if Logica can access sensitive data.
...
### 2) Run Logica security audits (read-only)

As part of the default read-only checks, run `logica security audit --deep`. Only offer alternatives if the user requests them:

1. `logica security audit` (faster, non-probing)
2. `logica security audit --json` (structured output)

Offer to apply Logica safe defaults (numbered):

1. `logica security audit --fix`

Be explicit that `--fix` only tightens Logica defaults and file permissions. It does not change host firewall, SSH, or OS update policies.
...
### 3) Check Logica version/update status (read-only)

As part of the default read-only checks, run `logica update status`.
...
- Credential hygiene notes (location of Logica creds, prefer disk encryption)
...
After Logica install or first hardening pass, run at least one baseline audit and version check:

- `logica security audit`
- `logica security audit --deep`
- `logica update status`

Ongoing monitoring is recommended. Use the Logica cron tool/CLI to schedule periodic audits (Gateway scheduler). Do not create scheduled tasks without explicit approval. Store outputs in a user-approved location and avoid secrets in logs.
...
1. “Do you want me to schedule periodic audits (e.g., daily/weekly) via `logica cron add`?”

If the user says yes, ask for:

- cadence (daily/weekly), preferred time window, and output location
- whether to also schedule `logica update status`
...
Before creating, `logica cron list` and match on exact `name`. If found, `logica cron edit <id> ...`.
If not found, `logica cron add --name <name> ...`.

Also offer a periodic version check so the user can decide when to update (numbered):

1. `logica update status` (preferred for source checkouts and channels)
2. `npm view logica version` (published npm version)

## Logica command accuracy

Use only supported commands and flags:

- `logica security audit [--deep] [--fix] [--json]`
- `logica status` / `logica status --deep`
- `logica health --json`
- `logica update status`
- `logica cron add|list|runs|run`

Do not invent CLI flags or imply Logica enforces host firewall/SSH policies.
...
Follow the durable-memory prompt format used by Logica compaction:
