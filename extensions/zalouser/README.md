...
Logica extension for Zalo Personal Account messaging via [zca-cli](https://zca-cli.dev).
...
```bash
logica channels login --channel zalouser
...
```bash
logica message send --channel zalouser --target <threadId> --message "Hello from Logica!"
```
...
```bash
logica channels login --channel zalouser              # Login via QR
logica channels login --channel zalouser --account work
logica channels status --probe
logica channels logout --channel zalouser
```

### Directory (IDs, contacts, groups)

```bash
logica directory self --channel zalouser
logica directory peers list --channel zalouser --query "name"
logica directory groups list --channel zalouser --query "work"
logica directory groups members --channel zalouser --group-id <id>
```
...
```bash
# Text
logica message send --channel zalouser --target <threadId> --message "message"

# Media (URL)
logica message send --channel zalouser --target <threadId> --message "caption" --media-url "https://example.com/img.jpg"
```

### Listener

The listener runs inside the Gateway when the channel is enabled. For debugging,
use `logica channels logs --channel zalouser` or run `zca listen` directly.
...
```bash
logica channels login --channel zalouser --account work
logica message send --channel zalouser --account work --target <id> --message "Hello"
