...
`logica.invoke --each`) without adding new Logica code per workflow.
...
This extension depends on Logica internal modules (the embedded agent runner).
It is intended to ship as a **bundled** Logica extension (like `lobster`) and
be enabled via `plugins.entries` + tool allowlists.

It is **not** currently designed to be copied into
`~/.logica/extensions` as a standalone plugin directory.
