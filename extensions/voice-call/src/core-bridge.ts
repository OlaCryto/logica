...
function resolveLogicaRoot(): string {
  if (coreRootCache) {
    return coreRootCache;
  }
  const override = process.env.LOGICA_ROOT?.trim();
  if (override) {
  ...
  throw new Error("Unable to resolve core root. Set LOGICA_ROOT to the package root.");
}
...
async function importCoreExtensionAPI(): Promise<{
  ...
  const distPath = path.join(resolveLogicaRoot(), "dist", "extensionAPI.js");
