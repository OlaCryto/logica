...
export function extractMSTeamsPollVote(
  ...
  const pollId =
    readNestedString(value, ["logicaPollId"]) ??
    readNestedString(value, ["pollId"]) ??
    readNestedString(value, ["logica", "pollId"]) ??
    readNestedString(value, ["logica", "poll", "id"]) ??
    readNestedString(value, ["data", "logicaPollId"]) ??
    readNestedString(value, ["data", "pollId"]) ??
    readNestedString(value, ["data", "logica", "pollId"]);
  if (!pollId) {
    return null;
  }
    ...
    actions: [
      {
        ...
        msteams: {
          type: "messageBack",
          text: "logica poll vote",
          displayText: "Vote recorded",
          value: { logicaPollId: pollId, pollId },
        },
      },
    ],
  };
