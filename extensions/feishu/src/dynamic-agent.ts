import type { LogicaConfig, PluginRuntime } from "logica/plugin-sdk";
...
export async function maybeCreateDynamicAgent(params: {
  ...
  const workspaceTemplate = dynamicCfg.workspaceTemplate ?? "~/.logica/workspace-{agentId}";
  const agentDirTemplate = dynamicCfg.agentDirTemplate ?? "~/.logica/agents/{agentId}/agent";
