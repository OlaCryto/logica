...
export async function handleIrcInbound(params: {
  ...
  const hasControlCommand = core.channel.text.hasControlCommand(rawBody, config as LogicaConfig);
  ...
  const mentionRegexes = core.channel.mentions.buildMentionRegexes(config as LogicaConfig);
  ...
  const envelopeOptions = core.channel.reply.resolveEnvelopeFormatOptions(config as LogicaConfig);
