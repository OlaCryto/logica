import type { LogicaPluginApi, LogicaPluginService } from "logica/plugin-sdk";
...
function normalizeDenyList(cfg: LogicaPluginApi["config"]): string[] {
  return uniqSorted([...(cfg.gateway?.nodes?.denyCommands ?? [])]);
}

function normalizeAllowList(cfg: LogicaPluginApi["config"]): string[] {
...
export default function register(api: LogicaPluginApi) {
  ...
  const timerService: LogicaPluginService = {
