import type { PluginRuntime } from "logica/plugin-sdk";
...
async function withTempStateDir<T>(fn: (dir: string) => Promise<T>) {
  const previous = process.env.LOGICA_STATE_DIR;
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "logica-nostr-"));
  process.env.LOGICA_STATE_DIR = dir;
  setNostrRuntime({
    state: {
      resolveStateDir: (env, homedir) => {
        ...
        return path.join(homedir(), ".logica");
      },
    },
  } as PluginRuntime);
  try {
    ...
    if (previous === undefined) {
      ...
      process.env.LOGICA_STATE_DIR = previous;
    }
