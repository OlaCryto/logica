...
import type { LogicaPluginApi, LogicaPluginToolContext } from "../../../src/plugins/types.js";
import { createLobsterTool } from "./lobster-tool.js";

async function writeFakeLobsterScript(scriptBody: string, prefix = "logica-lobster-plugin-") {
...
function fakeApi(overrides: Partial<LogicaPluginApi> = {}): LogicaPluginApi {
  return {
    id: "lobster",
    name: "lobster",
    source: "test",
...
function fakeCtx(overrides: Partial<LogicaPluginToolContext> = {}): LogicaPluginToolContext {
  return {
    config: {},
    workspaceDir: "/tmp",
    agentDir: "/tmp",
...
describe("lobster plugin tool", () => {
  ...
  it("can be gated off in sandboxed contexts", async () => {
    const api = fakeApi();
    const factoryTool = (ctx: LogicaPluginToolContext) => {
      if (ctx.sandboxed) {
        return null;
      }
