...
metadata: { "logica": { "emoji": "🪶", "homepage": "https://www.prose.md" } }
---
...
- **Task tool** in the upstream spec == Logica `sessions_spawn`
- **File I/O** == Logica `read`/`write`
- **Remote fetch** == Logica `web_fetch` (or `exec` with curl when POST is required)
