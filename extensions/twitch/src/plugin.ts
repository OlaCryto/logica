/**
 * Twitch channel plugin for Logica.
 *
...
import { buildChannelConfigSchema } from "logica/plugin-sdk";
import type {
 ...
 * for Logica. Supports message sending, receiving, access control, and
 * status monitoring.
 */
export const twitchPlugin: ChannelPlugin<TwitchAccountConfig> = {
  ...
  config: {
    ...
    listAccountIds: (cfg: LogicaConfig): string[] => listAccountIds(cfg),

    /** Resolve an account config by ID */
    resolveAccount: (cfg: LogicaConfig, accountId?: string | null): TwitchAccountConfig => {
    ...
    isConfigured: (_account: unknown, cfg: LogicaConfig): boolean => {
