...
import { approveDevicePairing, listDevicePairing } from "logica/plugin-sdk";
...
function resolveGatewayPort(cfg: LogicaPluginApi["config"]): number {
  const envRaw =
    process.env.LOGICA_GATEWAY_PORT?.trim() || process.env.CLAWDBOT_GATEWAY_PORT?.trim();
  if (envRaw) {
...
async function resolveTailnetHost(api: LogicaPluginApi): Promise<string | null> {
...
function resolveAuth(cfg: LogicaPluginApi["config"]): ResolveAuthResult {
...
async function resolveGatewayUrl(api: LogicaPluginApi): Promise<ResolveUrlResult> {
