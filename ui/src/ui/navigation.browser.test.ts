...
describe("control UI routing", () => {
  ...
  it("infers nested base paths", async () => {
    const app = mountApp("/apps/logica/cron");
    await app.updateComplete;

    expect(app.basePath).toBe("/apps/logica");
    expect(app.tab).toBe("cron");
    expect(window.location.pathname).toBe("/apps/logica/cron");
  });

  it("honors explicit base path overrides", async () => {
    window.__LOGICA_CONTROL_UI_BASE_PATH__ = "/logica";
    const app = mountApp("/logica/sessions");
    await app.updateComplete;

    expect(app.basePath).toBe("/logica");
    expect(app.tab).toBe("sessions");
    expect(window.location.pathname).toBe("/logica/sessions");
  });
