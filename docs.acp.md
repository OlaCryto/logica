...
This document describes how the Logica ACP (Agent Client Protocol) bridge works,
how it maps ACP sessions to Gateway sessions, and how IDEs should invoke it.

## Overview

`logica acp` exposes an ACP agent over stdio and forwards prompts to a running
Logica Gateway over WebSocket. It keeps ACP session ids mapped to Gateway
...
drive a Logica Gateway session.

Quick steps:

1. Run a Gateway (local or remote).
2. Configure the Gateway target (`gateway.remote.url` + auth) or pass flags.
3. Point the IDE to run `logica acp` over stdio.

Example config:

```bash
logica config set gateway.remote.url wss://gateway-host:18789
logica config set gateway.remote.token <token>
```

Example run:

```bash
logica acp --url wss://gateway-host:18789 --token <token>
```
...
```bash
logica acp --session agent:main:main
logica acp --session agent:design:main
logica acp --session agent:qa:bug-123
```
...
In Zed, open the Agent panel and select “Logica ACP” to start a thread.

## Execution Model

- ACP client spawns `logica acp` and speaks ACP messages over stdio.
...
```bash
logica acp --session agent:main:main
logica acp --session-label "support inbox"
logica acp --reset-session
```
...
`logica acp` resolves the Gateway URL and auth from CLI flags or config:
