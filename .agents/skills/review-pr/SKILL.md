...
- If you see "fatal: not a git repository", you are in the wrong directory. Use `~/dev/logica` if available; otherwise ask user.
...
Assume Logica subagents run with full disk access, including git, gh, and shell. Check auth, input validation, secrets, dependencies, tool safety, and privacy.
