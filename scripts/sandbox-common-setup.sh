...
BASE_IMAGE="${BASE_IMAGE:-logica-sandbox:bookworm-slim}"
TARGET_IMAGE="${TARGET_IMAGE:-logica-sandbox-common:bookworm-slim}"
...
If you want a clean re-create, remove old sandbox containers:
  docker rm -f \$(docker ps -aq --filter label=logica.sandbox=1)
NOTE
