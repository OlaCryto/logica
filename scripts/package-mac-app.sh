...
# Build and bundle Logica into a minimal .app we can open.
# Outputs to dist/Logica.app

ROOT_DIR="$(cd "$(dirname "$0")/.." && pwd)"
APP_ROOT="$ROOT_DIR/dist/Logica.app"
BUILD_ROOT="$ROOT_DIR/apps/macos/.build"
PRODUCT="Logica"
BUNDLE_ID="${BUNDLE_ID:-ai.logica.mac.debug}"
...
SPARKLE_FEED_URL="${SPARKLE_FEED_URL:-https://raw.githubusercontent.com/logica/logica/main/appcast.xml}"
AUTO_CHECKS=true
...
INFO_PLIST_SRC="$ROOT_DIR/apps/macos/Sources/Logica/Resources/Info.plist"
...
/usr/libexec/PlistBuddy -c "Set :LogicaBuildTimestamp ${BUILD_TS}" "$APP_ROOT/Contents/Info.plist" || true
/usr/libexec/PlistBuddy -c "Set :LogicaGitCommit ${GIT_COMMIT}" "$APP_ROOT/Contents/Info.plist" || true
...
echo "🚚 Copying binary"
cp "$BIN_PRIMARY" "$APP_ROOT/Contents/MacOS/Logica"
if [[ "${#BUILD_ARCHS[@]}" -gt 1 ]]; then
  BIN_INPUTS=()
  for arch in "${BUILD_ARCHS[@]}"; do
    BIN_INPUTS+=("$(bin_for_arch "$arch")")
  done
  /usr/bin/lipo -create "${BIN_INPUTS[@]}" -output "$APP_ROOT/Contents/MacOS/Logica"
fi
chmod +x "$APP_ROOT/Contents/MacOS/Logica"
# SwiftPM outputs ad-hoc signed binaries; strip the signature before install_name_tool to avoid warnings.
/usr/bin/codesign --remove-signature "$APP_ROOT/Contents/MacOS/Logica" 2>/dev/null || true
...
cp "$ROOT_DIR/apps/macos/Sources/Logica/Resources/Logica.icns" "$APP_ROOT/Contents/Resources/Logica.icns"

echo "📦 Copying device model resources"
rm -rf "$APP_ROOT/Contents/Resources/DeviceModels"
cp -R "$ROOT_DIR/apps/macos/Sources/Logica/Resources/DeviceModels" "$APP_ROOT/Contents/Resources/DeviceModels"
...
echo "📦 Copying LogicaKit resources"
LOGICAKIT_BUNDLE="$(build_path_for_arch "$PRIMARY_ARCH")/$BUILD_CONFIG/LogicaKit_LogicaKit.bundle"
if [ -d "$LOGICAKIT_BUNDLE" ]; then
  rm -rf "$APP_ROOT/Contents/Resources/LogicaKit_LogicaKit.bundle"
  cp -R "$LOGICAKIT_BUNDLE" "$APP_ROOT/Contents/Resources/LogicaKit_LogicaKit.bundle"
else
  echo "WARN: LogicaKit resource bundle not found at $LOGICAKIT_BUNDLE (continuing)" >&2
fi
...
echo "⏹  Stopping any running Logica"
killall -q Logica 2>/dev/null || true
