...
#
# Suggested cron: */30 * * * * /home/admin/logica/scripts/auth-monitor.sh
#
# Environment variables:
#   NOTIFY_PHONE - Phone number to send Logica notification (e.g., +1234567890)
#   NOTIFY_NTFY  - ntfy.sh topic for push notifications (e.g., logica-alerts)
...
STATE_FILE="$HOME/.logica/auth-monitor-state"
...
send_notification() {
    ...
    # Send via Logica if phone configured and auth still valid
    if [ -n "$NOTIFY_PHONE" ]; then
        # Check if we can still use logica
        if "$SCRIPT_DIR/claude-auth-status.sh" simple 2>/dev/null | grep -q "OK\|EXPIRING"; then
            echo "Sending via Logica to $NOTIFY_PHONE..."
            logica send --to "$NOTIFY_PHONE" --message "$message" 2>/dev/null || true
        fi
    fi
...
if [ "$DIFF_MS" -lt 0 ]; then
    send_notification "Claude Code auth EXPIRED! Logica is down. Run: ssh l36 '~/logica/scripts/mobile-reauth.sh'" "urgent"
    exit 1
