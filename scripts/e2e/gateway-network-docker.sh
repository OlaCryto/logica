...
IMAGE_NAME="logica-gateway-network-e2e"

PORT="18789"
TOKEN="e2e-$(date +%s)-$$"
NET_NAME="logica-net-e2e-$$"
GW_NAME="logica-gateway-e2e-$$"
...
echo "Starting gateway container..."
	docker run --rm -d \
	  --name "$GW_NAME" \
	  --network "$NET_NAME" \
	  -e "LOGICA_GATEWAY_TOKEN=$TOKEN" \
	  -e "LOGICA_SKIP_CHANNELS=1" \
	  -e "LOGICA_SKIP_GMAIL_WATCHER=1" \
	  -e "LOGICA_SKIP_CRON=1" \
	  -e "LOGICA_SKIP_CANVAS_HOST=1" \
	  "$IMAGE_NAME" \
