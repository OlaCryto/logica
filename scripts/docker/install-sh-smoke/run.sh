...
INSTALL_URL="${LOGICA_INSTALL_URL:-https://logica.bot/install.sh}"
SMOKE_PREVIOUS_VERSION="${LOGICA_INSTALL_SMOKE_PREVIOUS:-}"
SKIP_PREVIOUS="${LOGICA_INSTALL_SMOKE_SKIP_PREVIOUS:-0}"
DEFAULT_PACKAGE="logica"
PACKAGE_NAME="${LOGICA_INSTALL_PACKAGE:-$DEFAULT_PACKAGE}"
...
if [[ "$SKIP_PREVIOUS" == "1" ]]; then
  echo "==> Skip preinstall previous (LOGICA_INSTALL_SMOKE_SKIP_PREVIOUS=1)"
else
...
if [[ -n "${LOGICA_INSTALL_LATEST_OUT:-}" ]]; then
  printf "%s" "$LATEST_VERSION" > "${LOGICA_INSTALL_LATEST_OUT:-}"
fi
