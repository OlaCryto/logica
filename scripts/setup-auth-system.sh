#!/bin/bash
# Setup Logica Auth Management System
...
echo "  2. Subscribe to a topic (e.g., 'logica-alerts')"
echo ""
...
# Phone notification
echo ""
echo "  Logica message: Send warning via Logica itself"
echo "Enter your phone number for alerts (or leave blank to skip):"
read -r PHONE_NUMBER

# Update service file
SERVICE_FILE="$SCRIPT_DIR/systemd/logica-auth-monitor.service"
...
cp "$SCRIPT_DIR/systemd/logica-auth-monitor.service" ~/.config/systemd/user/
cp "$SCRIPT_DIR/systemd/logica-auth-monitor.timer" ~/.config/systemd/user/
systemctl --user daemon-reload
systemctl --user enable --now logica-auth-monitor.timer
...
echo "  - Auth monitor: systemctl --user status logica-auth-monitor.timer"
echo ""
echo "Quick commands:"
