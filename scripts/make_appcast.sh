...
ZIP=${1:?"Usage: $0 Logica-<ver>.zip"}
FEED_URL=${2:-"https://raw.githubusercontent.com/logica/logica/main/appcast.xml"}
...
if [[ -z "$VERSION" ]]; then
  if [[ "$ZIP_NAME" =~ ^Logica-([0-9]+(\.[0-9]+){1,2}([-.][^.]*)?)\.zip$ ]]; then
...
DOWNLOAD_URL_PREFIX=${SPARKLE_DOWNLOAD_URL_PREFIX:-"https://github.com/logica/logica/releases/download/v${VERSION}/"}
