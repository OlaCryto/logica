...
STATUS=$(ssh -o ConnectTimeout=5 "$SERVER" '$HOME/logica/scripts/claude-auth-status.sh simple' 2>&1)
...
case "$STATUS" in
    ...
    *EXPIRED*|*MISSING*)
        ...
        sleep 2
        termux-notification -t "Logica Re-Auth" -c "After getting key, run: ssh $SERVER '~/logica/scripts/mobile-reauth.sh'" --id logica-auth
        ;;
    *)
