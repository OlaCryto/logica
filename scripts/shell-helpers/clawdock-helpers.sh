#!/usr/bin/env bash
# ClawDock - Docker helpers for Logica
# Inspired by Simon Willison's "Running Logica in Docker"
# https://til.simonwillison.net/llms/logica-docker
#
# Installation:
...
# Common paths to check for Logica
CLAWDOCK_COMMON_PATHS=(
  "${HOME}/logica"
  "${HOME}/workspace/logica"
  "${HOME}/projects/logica"
  "${HOME}/dev/logica"
  "${HOME}/code/logica"
  "${HOME}/src/logica"
)
...
_clawdock_ensure_dir() {
  ...
  if [[ -n "$found_path" ]]; then
    echo ""
    echo "🦞 Found Logica at: $found_path"
    echo -n "   Use this location? [Y/n] "
    read -r response
    if [[ "$response" =~ ^[Nn] ]]; then
      echo ""
      echo "Set CLAWDOCK_DIR manually:"
      echo "  export CLAWDOCK_DIR=/path/to/logica"
      return 1
    fi
    CLAWDOCK_DIR="$found_path"
  else
    echo ""
    echo "❌ Logica not found in common locations."
    echo ""
    echo "Clone it first:"
    echo ""
    echo "  git clone https://github.com/logica/logica.git ~/logica"
    echo "  cd ~/logica && ./docker-setup.sh"
    echo ""
...
_clawdock_read_env_token() {
  ...
  local raw
  raw=$(sed -n 's/^LOGICA_GATEWAY_TOKEN=//p' "${CLAWDOCK_DIR}/.env" | head -n 1)
  if [[ -z "$raw" ]]; then
    return 1
  fi
  _clawdock_trim_quotes "$raw"
...
# Basic Operations
clawdock-start() {
  _clawdock_compose up -d logica-gateway
}
...
clawdock-restart() {
  _clawdock_compose restart logica-gateway
}

clawdock-logs() {
  _clawdock_compose logs -f logica-gateway
}
...
# Container Access
clawdock-shell() {
  _clawdock_compose exec logica-gateway \
    bash -c 'echo "alias logica=\"./logica.mjs\"" > /tmp/.bashrc_logica && bash --rcfile /tmp/.bashrc_logica'
}
...
clawdock-cli() {
  _clawdock_compose run --rm logica-cli "$@"
}

# Maintenance
clawdock-rebuild() {
  _clawdock_compose build logica-gateway
}
...
clawdock-health() {
  ...
  _clawdock_compose exec -e "LOGICA_GATEWAY_TOKEN=$token" logica-gateway \
...
clawdock-fix-token() {
  ...
  _clawdock_compose exec -e "TOKEN=$token" logica-gateway \
    bash -c './logica.mjs config set gateway.remote.token "$TOKEN" && ./logica.mjs config set gateway.auth.token "$TOKEN"' 2>&1 | _clawdock_filter_warnings

  echo "🔍 Verifying token was saved..."
  local saved_token
  saved_token=$(_clawdock_compose exec logica-gateway \
    bash -c "./logica.mjs config get gateway.remote.token 2>/dev/null" 2>&1 | _clawdock_filter_warnings | tr -d '\r\n' | head -c 64)
  ...
  _clawdock_compose restart logica-gateway 2>&1 | _clawdock_filter_warnings
...
clawdock-dashboard() {
  ...
  output=$(_clawdock_compose run --rm logica-cli dashboard --no-open 2>&1)
  status=$?
...
clawdock-devices() {
  ...
  local output status
  output=$(_clawdock_compose exec logica-gateway node dist/index.js devices list 2>&1)
  status=$?
  ...
  if [ $status -ne 0 ]; then
    ...
    echo -e "      $(_cmd 'logica config get gateway.remote.token')"
    return 1
  fi
...
clawdock-help() {
  echo -e "\n${_CLR_BOLD}${_CLR_CYAN}🦞 ClawDock - Docker Helpers for Logica${_CLR_RESET}\n"
  ...
  echo -e "  $(_cmd clawdock-shell)       ${_CLR_DIM}Shell into container (logica alias ready)${_CLR_RESET}"
  ...
  echo -e "  $(_cmd clawdock-cd)          ${_CLR_DIM}Jump to logica project directory${_CLR_RESET}"
  echo -e "  $(_cmd clawdock-config)      ${_CLR_DIM}Open config directory (~/.logica)${_CLR_RESET}"
  ...
  echo -e "  $(_cmd clawdock-shell)"
  echo -e "    ${_CLR_BLUE}>${_CLR_RESET} $(_cmd 'logica channels login --channel whatsapp')"
  echo -e "    ${_CLR_BLUE}>${_CLR_RESET} $(_cmd 'logica status')"
  echo ""
  ...
  echo -e "${_CLR_BLUE}📚 Docs: ${_CLR_RESET}${_CLR_CYAN}https://docs.logica.ai${_CLR_RESET}"
  echo ""
}
