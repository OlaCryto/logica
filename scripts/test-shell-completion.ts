 ...
 * `logica update`. Use it to verify the completion installation flow
...
\`logica update\`, \`logica doctor\`, and \`logica onboard\`.
...
async function main() {
  ...
  if (status.profileInstalled && status.cacheExists && !options.force) {
    ...
    console.log(
      theme.muted("  1. Remove the '# Logica Completion' block from your shell profile"),
    );
  ...
  if (isCancel(shouldInstall) || !shouldInstall) {
    console.log(theme.muted(`Skipped. Run \`logica completion --install\` later to enable.`));
    return;
  }
