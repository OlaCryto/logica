...
read_when:
  - Designing workspace memory (~/.logica/workspace) beyond daily Markdown logs
  - Deciding: standalone CLI vs deep Logica integration
...
Target: Clawd-style workspace (`agents.defaults.workspace`, default `~/.logica/workspace`) where “memory” is stored as one Markdown file per day (`memory/YYYY-MM-DD.md`) plus a small set of stable files (e.g. `memory.md`, `SOUL.md`).
...
Keep `~/.logica/workspace` as canonical human-readable memory.
...
```
~/.logica/workspace/.memory/index.sqlite
```
...
Recommendation: **deep integration in Logica**, but keep a separable core library.

### Why integrate into Logica?

- Logica already knows:
  - the workspace path (`agents.defaults.workspace`)
  - the session model + heartbeats
  - logging + troubleshooting patterns
- You want the agent itself to call the tools:
  - `logica memory recall "…" --k 25 --since 30d`
  - `logica memory reflect --since 7d`
...
Pragmatic take for `~/.logica/workspace`:
