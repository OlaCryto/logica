---
summary: "Use Venice AI privacy-focused models in Logica"
read_when:
  - You want privacy-focused inference in Logica
...
```bash
logica onboard --auth-choice venice-api-key
```
...
```bash
logica chat --model venice/llama-3.3-70b "Hello, are you working?"
```

## Model Selection

After setup, Logica shows all available Venice models. Pick based on your needs:
...
```bash
logica models set venice/claude-opus-45
logica models set venice/llama-3.3-70b
```

List all available models:

```bash
logica models list | grep venice
```

## Configure via `logica configure`

1. Run `logica configure`
2. Select **Model/auth**
3. Choose **Venice AI**
...
Logica automatically discovers models from the Venice API when `VENICE_API_KEY` is set. If the API is unreachable, it falls back to a static catalog.
...
logica chat --model venice/llama-3.3-70b

# Use Claude via Venice (anonymized)
logica chat --model venice/claude-opus-45

# Use uncensored model
logica chat --model venice/venice-uncensored

# Use vision model with image
logica chat --model venice/qwen3-vl-235b-a22b

# Use coding model
logica chat --model venice/qwen3-coder-480b-a35b-instruct
```
...
The Venice model catalog updates dynamically. Run `logica models list` to see currently available models. Some models may be temporarily offline.
