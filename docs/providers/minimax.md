---
summary: "Use MiniMax M2.1 in Logica"
read_when:
  - You want MiniMax models in Logica
...
```bash
logica plugins enable minimax-portal-auth  # skip if already loaded.
logica gateway restart  # restart if gateway is already running
logica onboard --auth-choice minimax-portal
```
...
See [MiniMax OAuth plugin README](https://github.com/logica/logica/tree/main/extensions/minimax-portal-auth) for details.
...
Configure manually via `logica.json`:
...
- Use `logica models list` and `logica models set minimax/MiniMax-M2.1` to switch.
...
- Running `logica configure` and selecting **MiniMax M2.1**, or
