...
- `/usage cost` in chats: local cost summary aggregated from Logica session logs.
- CLI: `logica status --usage` prints a full per-provider breakdown.
- CLI: `logica channels list` prints the same usage snapshot alongside provider config (use `--no-usage` to skip).
