...
read_when:
  - You want to understand what “context” means in Logica
...
“Context” is **everything Logica sends to the model for a run**. It is bounded by the model’s **context window** (token limit).

Beginner mental model:

- **System prompt** (Logica-built): rules, tools, skills list, time/runtime, and injected workspace files.
...
## How Logica builds the system prompt

The system prompt is **Logica-owned** and rebuilt each run. It includes:
...
By default, Logica injects a fixed set of workspace files (if present):
