...
read_when:
  ...
  - Investigating tsx/esbuild loader crashes in Logica
...
Running Logica via Node with `tsx` fails at startup with:

```
[logica] Failed to start CLI: TypeError: __name is not a function
...
- Before that (Bun path), `logica status` and `gateway:watch` worked.
...
- Use Node + tsc watch, then run compiled output:
  ...
  node --watch logica.mjs status
  ```

- Confirmed locally: `pnpm exec tsc -p tsconfig.json` + `node logica.mjs status` works on Node 25.
