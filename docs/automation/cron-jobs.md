...
- Jobs persist under `~/.logica/cron/` so restarts don’t lose schedules.
...
logica cron list
logica cron run <job-id>
logica cron runs --id <job-id>
```
...
Cron jobs are persisted on the Gateway host at `~/.logica/cron/jobs.json` by default.
The Gateway loads the file into memory and writes it back on changes, so manual edits
are only safe when the Gateway is stopped. Prefer `logica cron add/edit` or the cron
...
If `delivery` is omitted for isolated jobs, Logica defaults to `announce`.
...
- Job store: `~/.logica/cron/jobs.json` (Gateway-managed JSON).
- Run history: `~/.logica/cron/runs/<jobId>.jsonl` (JSONL, auto-pruned).
...
logica cron add --name "Ops sweep" --cron "0 6 * * *" --session isolated --message "Check ops queue" --agent ops

# Switch or clear the agent on an existing job
logica cron edit <jobId> --agent ops
logica cron edit <jobId> --clear-agent
```

Manual run (force is the default, use `--due` to only run when due):

```bash
logica cron run <jobId>
logica cron run <jobId> --due
```

Edit an existing job (patch fields):

```bash
logica cron edit <jobId> \
...
```bash
logica system event --mode now --text "Next heartbeat: check battery."
```
...
- `cron.run` (force or due), `cron.runs`
  For immediate system events without a job, use [`logica system event`](/cli/system).

## Troubleshooting

### “Nothing runs”

- Check cron is enabled: `cron.enabled` and `LOGICA_SKIP_CRON`.
- Check the Gateway is running continuously (cron runs inside the Gateway process).
- For `cron` schedules: confirm timezone (`--tz`) vs the host timezone.

### A recurring job keeps delaying after failures

- Logica applies exponential retry backoff for recurring jobs after consecutive errors:
