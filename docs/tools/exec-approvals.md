...
- **gateway host** → `logica` process on the gateway machine
...
`~/.logica/exec-approvals.json` directly.

CLI: `logica approvals` supports gateway or node editing (see [Approvals CLI](/cli/approvals)).
