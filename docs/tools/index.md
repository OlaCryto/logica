---
summary: "Agent tool surface for Logica (browser, canvas, nodes, message, cron) replacing legacy `logica-*` skills"
read_when:
  - Adding or modifying agent tools
  - Retiring or changing `logica-*` skills
title: "Tools"
---

# Tools (Logica)

Logica exposes **first-class agent tools** for browser, canvas, nodes, and cron.
These replace the old `logica-*` skills: the tools are typed, no shelling,
and the agent should rely on them directly.

## Disabling tools

You can globally allow/deny tools via `tools.allow` / `tools.deny` in `logica.json`
...
- If `tools.allow` only references unknown or unloaded plugin tool names, Logica logs a warning and ignores the allowlist so core tools stay available.
...
- `host=node` can target a macOS companion app or a headless node host (`logica node run`).
...
- Requires a Brave API key (recommended: `logica configure --section web`, or set `BRAVE_API_KEY`).
...
Control the dedicated Logica-managed browser.
...
- Quick smoke: `logica nodes canvas a2ui push --node <id> --text "Hello from A2UI"`.
...
- `restart` (authorizes + sends `SIGUSR1` for in-process restart; `logica gateway` restart in-place)
