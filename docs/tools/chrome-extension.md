---
summary: "Chrome extension: let Logica drive your existing Chrome tab"
read_when:
...
The Logica Chrome extension lets the agent control your **existing Chrome tabs** (your normal Chrome window) instead of launching a separate logica-managed Chrome profile.
...
Logica then controls the attached tab through the normal `browser` tool surface (selecting the right profile).
...
The extension ships inside the Logica release (npm package) as static files. There is no separate “build” step.

After upgrading Logica:

- Re-run `logica browser extension install` to refresh the installed files under your Logica state directory.
- Chrome → `chrome://extensions` → click “Reload” on the extension.

## Use it (no extra config)

Logica ships with a built-in browser profile named `chrome` that targets the extension relay on the default port.
...
- Open the tab you want Logica to control.
...
- `ON`: attached; Logica can drive that tab.
...
Debugging: `logica sandbox explain`
...
`logica browser extension path` prints the **installed** on-disk directory containing the extension files.

The CLI intentionally does **not** print a `node_modules` path. Always run `logica browser extension install` first to copy the extension to a stable location under your Logica state directory.
...
- **This is not isolated** like the dedicated logica-managed profile.
