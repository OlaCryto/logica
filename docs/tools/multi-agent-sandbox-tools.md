...
For debugging “why is this blocked?”, see [Sandbox vs Tool Policy vs Elevated](/gateway/sandbox-vs-tool-policy-vs-elevated) and `logica sandbox explain`.
...
Legacy `agent.*` configs are migrated by `logica doctor`; prefer `agents.defaults` + `agents.list` going forward.
...
2. **Verify sandbox containers:**
   ...
   ```exec
   docker ps --filter "name=logica-sbx-"
   ```
...
4. **Monitor logs:**
   ...
   ```exec
   tail -f "${LOGICA_STATE_DIR:-$HOME/.logica}/logs/gateway.log" | grep -E "routing|sandbox|tools"
   ```
