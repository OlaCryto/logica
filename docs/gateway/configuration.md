---
summary: "All configuration options for ~/.logica/logica.json with examples"
read_when:
...
Logica reads an optional **JSON5** config from `~/.logica/logica.json` (comments + trailing commas allowed).

If the file is missing, Logica uses safe-ish defaults (embedded Pi agent + per-sender sessions + workspace `~/.logica/workspace`). You usually only need a config to:
...
Logica only accepts configurations that fully match the schema.
Unknown keys, malformed types, or invalid values cause the Gateway to **refuse to start** for safety.

When validation fails:

- The Gateway does not boot.
- Only diagnostic commands are allowed (for example: `logica doctor`, `logica logs`, `logica health`, `logica status`, `logica service`, `logica help`).
- Run `logica doctor` to see the exact issues.
- Run `logica doctor --fix` (or `--yes`) to apply migrations/repairs.
...
use `config.patch` or `logica config set`. Keep a backup of `~/.logica/logica.json`.
...
```bash
logica gateway call config.get --params '{}' # capture payload.hash
logica gateway call config.apply --params '{
...
```bash
logica gateway call config.get --params '{}' # capture payload.hash
logica gateway call config.patch --params '{
...
{
  agents: {
    defaults: { workspace: "~/.logica/workspace" },
    list: [
      {
        id: "main",
        groupChat: { mentionPatterns: ["@logica", "reisponde"] },
      },
    ],
  },
  channels: {
...
// ~/.logica/agents.json5
{
  ...
  list: [{ id: "main", workspace: "~/.logica/workspace" }],
}
```
...
{ "$include": "/etc/logica/base.json5" } // absolute
...
```json5
// ~/.logica/clients/mueller/agents.json5
[
...
```json5
// ~/.logica/clients/mueller/broadcast.json5
{
...
Logica reads env vars from the parent process (shell, launchd/systemd, CI, etc.).
...
Opt-in convenience: if enabled and none of the expected keys are set yet, Logica runs your login shell and imports only the missing expected keys (never overrides).
...
- `LOGICA_LOAD_SHELL_ENV=1`
- `LOGICA_SHELL_ENV_TIMEOUT_MS=15000`
...
Logica stores **per-agent** auth profiles (OAuth + API keys) in:

- `<agentDir>/auth-profiles.json` (default: `~/.logica/agents/<agentId>/agent/auth-profiles.json`)

See also: [/concepts/oauth](/concepts/oauth)

Legacy OAuth imports:

- `~/.logica/credentials/oauth.json` (or `$LOGICA_STATE_DIR/credentials/oauth.json`)
...
- `~/.logica/agent/*` (migrated by `logica doctor` into `~/.logica/agents/<defaultAgentId>/agent/*`)

Overrides:

- OAuth dir (legacy import only): `LOGICA_OAUTH_DIR`
- Agent dir (default agent root override): `LOGICA_AGENT_DIR` (preferred), `PI_CODING_AGENT_DIR` (legacy)

On first use, Logica imports `oauth.json` entries into `auth-profiles.json`.
...
If set, Logica derives defaults (only when you haven’t set them explicitly):
...
- Default log file: `/tmp/logica/logica-YYYY-MM-DD.log`
- If you want a stable path, set `logging.file` to `/tmp/logica/logica.log`.
  ...
  channels: {
    whatsapp: {
      accounts: {
        ...
        personal: {},
        biz: {
          // Optional override. Default: ~/.logica/credentials/whatsapp/biz
          // authDir: "~/.logica/credentials/whatsapp/biz",
        },
      },
    },
  },
...
- The legacy single-account Baileys auth dir is migrated by `logica doctor` into `whatsapp/default`.
...
```json5
{
  ...
  agents: {
    list: [{ id: "main", groupChat: { mentionPatterns: ["@logica", "logica"] } }],
  },
}
```
    ...
    list: [
      {
        ...
        groupChat: {
          ...
          mentionPatterns: ["reisponde", "@logica"],
        },
      },
    ],
  },
...
- `agents.list[]`: per-agent overrides.
  ...
  - `workspace`: default `~/.logica/workspace-<agentId>` (for `main`, falls back to `agents.defaults.workspace`).
  - `agentDir`: default `~/.logica/agents/<agentId>/agent`.
...
```json5
{
  agents: {
    list: [
      {
        id: "personal",
        workspace: "~/.logica/workspace-personal",
        sandbox: { mode: "off" },
      },
    ],
  },
...
```json5
{
  agents: {
    list: [
      {
        id: "family",
        workspace: "~/.logica/workspace-family",
        sandbox: {
          mode: "all",
          scope: "agent",
          workspaceAccess: "ro",
...
```json5
{
  agents: {
    list: [
      {
        id: "public",
        workspace: "~/.logica/workspace-public",
        sandbox: {
          mode: "all",
          scope: "agent",
...
```json5
{
  agents: {
    list: [
      { id: "home", default: true, workspace: "~/.logica/workspace-home" },
      { id: "work", workspace: "~/.logica/workspace-work" },
    ],
  },
  bindings: [
...
- `commands.config: true` enables `/config` (reads/writes `logica.json`).
...
Logica starts Telegram only when a `channels.telegram` config section exists. The bot token is resolved from `channels.telegram.botToken` (or `channels.telegram.tokenFile`), with `TELEGRAM_BOT_TOKEN` as a fallback for the default account.
  ...
  channels: {
    discord: {
      ...
      dm: {
        ...
        groupChannels: ["logica-dm"], // optional group DM allowlist
      },
      guilds: {
        "123456789012345678": {
          // guild id (preferred) or slug
          slug: "friends-of-logica",
...
Logica starts Slack when the provider is enabled and both tokens are set (via config or `SLACK_BOT_TOKEN` + `SLACK_APP_TOKEN`). Use `user:<id>` (DM) or `channel:<id>` when specifying delivery targets for cron/CLI commands.
...
Install it first: `logica plugins install @logica/mattermost` (or `./extensions/mattermost` from a git checkout).
...
Logica starts Mattermost when the account is configured (bot token + base URL) and enabled. The token + base URL are resolved from `channels.mattermost.botToken` + `channels.mattermost.baseUrl` or `MATTERMOST_BOT_TOKEN` + `MATTERMOST_URL` for the default account (unless `channels.mattermost.enabled` is `false`).
...
Logica spawns `imsg rpc` (JSON-RPC over stdio). No daemon or port required.
...
Optional repository root to show in the system prompt’s Runtime line. If unset, Logica
...
```json5
{
  agents: { defaults: { repoRoot: "~/Projects/logica" } },
}
```
...
When a file exceeds this limit, Logica logs a warning and injects a truncated
...
message envelopes). If unset, Logica uses the host timezone at runtime.
...
`[logica]`, so same-phone conversations stay legible.
...
`messages.messagePrefix`). Default stays **unchanged**: `"[logica]"` when
`channels.whatsapp.allowFrom` is empty, otherwise `""` (no prefix). When using
`"[logica]"`, Logica will instead use `[{identity.name}]` when the routed
...
Enable text-to-speech for outbound replies. When on, Logica generates audio
...
```json5
{
  messages: {
    tts: {
      ...
      maxTextLength: 4000,
      timeoutMs: 30000,
      prefsPath: "~/.logica/settings/tts.json",
      elevenlabs: {
...
Logica also ships a few built-in alias shorthands. Defaults only apply when the model
...
If you omit the provider, Logica currently assumes `anthropic` as a temporary
...
- `tools.web.search.apiKey` (recommended: set via `logica configure --section web`, or use `BRAVE_API_KEY` env var)
...
- `group:nodes`: `nodes`
- `group:logica`: all built-in Logica tools (excludes provider plugins)
...
- agent workspace access: `workspaceAccess: "none"` (default)
  - `"none"`: use a per-scope sandbox workspace under `~/.logica/sandboxes`
  ...
  agents: {
    defaults: {
      sandbox: {
        ...
        workspaceRoot: "~/.logica/sandboxes",
        docker: {
          image: "logica-sandbox:bookworm-slim",
          containerPrefix: "logica-sbx-",
          workdir: "/workspace",
          readOnlyRoot: true,
          ...
          apparmorProfile: "logica-sandbox",
          dns: ["1.1.1.1", "8.8.8.8"],
        ...
        browser: {
          enabled: false,
          image: "logica-sandbox-browser:bookworm-slim",
          containerPrefix: "logica-sbx-browser-",
          cdpPort: 9222,
          vncPort: 5900,
          noVncPort: 6080,
          headless: false,
...
Logica uses the **pi-coding-agent** model catalog. You can add custom providers
(LiteLLM, local OpenAI-compatible servers, Anthropic proxies, etc.) by writing
`~/.logica/agents/<agentId>/agent/models.json` or by defining the same schema inside your
Logica config under `models.providers`.
Provider-by-provider overview + examples: [/concepts/model-providers](/concepts/model-providers).

When `models.providers` is present, Logica writes/merges a `models.json` into
`~/.logica/agents/<agentId>/agent/` on startup:
...
OpenCode Zen is a multi-model gateway with per-model endpoints. Logica uses
...
- Shortcut: `logica onboard --auth-choice opencode-zen`.
...
Shortcut: `logica onboard --auth-choice zai-api-key`.
...
- Set `MOONSHOT_API_KEY` in the environment or use `logica onboard --auth-choice moonshot-api-key`.
- Model ref: `moonshot/kimi-k2.5`.
- For the China endpoint, either:
  - Run `logica onboard --auth-choice moonshot-api-key-cn` (wizard will set `https://api.moonshot.cn/v1`), or
...
- Set `KIMI_API_KEY` in the environment or use `logica onboard --auth-choice kimi-code-api-key`.
...
- Set `SYNTHETIC_API_KEY` or use `logica onboard --auth-choice synthetic-api-key`.
...
- Set `MINIMAX_API_KEY` environment variable or use `logica onboard --auth-choice minimax-api`.
...
- Override the agent config root with `LOGICA_AGENT_DIR` (or `PI_CODING_AGENT_DIR`)
  if you want `models.json` stored elsewhere (default: `~/.logica/agents/main/agent`).
...
```json5
{
  session: {
    ...
    // Default is already per-agent under ~/.logica/agents/<agentId>/sessions/sessions.json
    // You can override with {agentId} templating:
    store: "~/.logica/agents/{agentId}/sessions/sessions.json",
    maintenance: {
      mode: "warn",
      pruneAfter: "30d",
      maxEntries: 500,
...
- `resetByType`: per-session overrides for `direct`, `group`, and `thread`. Legacy `dm` key is accepted as an alias for `direct`.
  - If you only set legacy `session.idleMinutes` without any `reset`/`resetByType`, Logica stays in idle-only mode for backward compatibility.
...
overrides. Applies to **bundled** skills and `~/.logica/skills` (workspace skills
...
from `~/.logica/extensions`, `<workspace>/.logica/extensions`, plus any
...
### `browser` (logica-managed browser)

Logica can start a **dedicated, isolated** Chrome/Brave/Edge/Chromium instance for logica and expose a small loopback control service.
...
- Note: the control server is started by the running gateway (Logica.app menubar, or `logica gateway`).
...
```json5
{
  browser: {
    ...
    profiles: {
      logica: { cdpPort: 18800, color: "#FF4500" },
...
```json5
{
  gateway: {
    ...
    bind: "loopback",
    // controlUi: { enabled: true, basePath: "/logica" }
...
- Examples: `"/ui"`, `"/logica"`, `"/apps/logica"`.
...
- When a connection comes from one of these IPs, Logica uses `x-forwarded-for` (or `x-real-ip`) to determine the client IP for local pairing checks and HTTP auth/local checks.
- Only list proxies you fully control, and ensure they **overwrite** incoming `x-forwarded-for`.

Notes:

- `logica gateway` refuses to start unless `gateway.mode` is set to `local` (or you pass the override flag).
- `gateway.port` controls the single multiplexed port used for WebSocket + HTTP (control UI, hooks, A2UI).
- OpenAI Chat Completions endpoint: **disabled by default**; enable with `gateway.http.endpoints.chatCompletions.enabled: true`.
- Precedence: `--port` > `LOGICA_GATEWAY_PORT` > `gateway.port` > default `18789`.
...
- `gateway.auth.password` can be set here, or via `LOGICA_GATEWAY_PASSWORD` (recommended).
- `gateway.auth.allowTailscale` allows Tailscale Serve identity headers
  (`tailscale-user-login`) to satisfy auth when the request arrives on loopback
  with `x-forwarded-for`, `x-forwarded-proto`, and `x-forwarded-host`. Logica
...
- Logica.app watches `~/.logica/logica.json` and switches modes live when `gateway.mode` or `gateway.remote.url` changes.
...
The Gateway watches `~/.logica/logica.json` (or `LOGICA_CONFIG_PATH`) and applies changes automatically.
...
- `~/.logica/logica.json` (or `LOGICA_CONFIG_PATH`)
...
- `LOGICA_STATE_DIR` (sessions/creds)
- `agents.defaults.workspace` (memories)
- `gateway.port` (unique per instance)

Convenience flags (CLI):

- `logica --dev …` → uses `~/.logica-dev` + shifts ports from base `19001`
- `logica --profile <name> …` → uses `~/.logica-<name>` (port via config/env/flags)
...
```bash
LOGICA_CONFIG_PATH=~/.logica/a.json \
LOGICA_STATE_DIR=~/.logica-a \
logica gateway --port 19001
```
...
Gmail helper config (used by `logica webhooks gmail setup` / `run`):

```json5
{
  hooks: {
    gmail: {
      account: "logica@gmail.com",
...
- Set `LOGICA_SKIP_GMAIL_WATCHER=1` to disable the auto-start (for manual runs).
...
Note: when `tailscale.mode` is on, Logica defaults `serve.path` to `/` so
...
Default root: `~/.logica/workspace/canvas`  
Default port: `18793` (chosen to avoid the logica browser CDP port `18792`)  
...
- watches the directory and broadcasts reloads over a WebSocket endpoint at `/__logica__/ws`
- auto-creates a starter `index.html` when the directory is empty (so you see something immediately)
- also serves A2UI at `/__logica__/a2ui/` and is advertised to nodes as `canvasHostUrl`
...
```json5
{
  bridge: {
    ...
    tls: {
      enabled: true,
      // Uses ~/.logica/bridge/tls/bridge-{cert,key}.pem when omitted.
      // certPath: "~/.logica/bridge/tls/bridge-cert.pem",
      // keyPath: "~/.logica/bridge/tls/bridge-key.pem"
    },
  },
}
```
...
Controls LAN mDNS discovery broadcasts (`_logica-gw._tcp`).

- `minimal` (default): omit `cliPath` + `sshPort` from TXT records
- `full`: include `cliPath` + `sshPort` in TXT records
- `off`: disable mDNS broadcasts entirely
- Hostname: defaults to `logica` (advertises `logica.local`). Override with `LOGICA_MDNS_HOSTNAME`.
...
When enabled, the Gateway writes a unicast DNS-SD zone for `_logica-gw._tcp` under `~/.logica/dns/` using the configured discovery domain (example: `logica.internal.`).
