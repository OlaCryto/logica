...
- Replaces the legacy `gateway` command. CLI entry point: `logica gateway`.
- Runs until stopped; exits non-zero on fatal errors so the supervisor restarts it.

## How to run (local)

```bash
logica gateway --port 18789
# for full debug/trace logs in stdio:
logica gateway --port 18789 --verbose
# if the port is busy, terminate listeners then start:
logica gateway --force
# dev loop (auto-reload on TS changes):
pnpm gateway:watch
```

- Config hot reload watches `~/.logica/logica.json` (or `LOGICA_CONFIG_PATH`).
...
- Starts a Canvas file server by default on `canvasHost.port` (default `18793`), serving `http://<gateway-host>:18793/__logica__/canvas/` from `~/.logica/workspace/canvas`. Disable with `canvasHost.enabled=false` or `LOGICA_SKIP_CANVAS_HOST=1`.
...
- Gateway auth is required by default: set `gateway.auth.token` (or `LOGICA_GATEWAY_TOKEN`) or `gateway.auth.password`. Clients must send `connect.params.auth.token/password` unless using Tailscale Serve identity.
- The wizard now generates a token by default, even on loopback.
- Port precedence: `--port` > `LOGICA_GATEWAY_PORT` > `gateway.port` > default `18789`.
...
- macOS: `bot.molt.<profile>` (legacy `com.logica.*` may still exist)
- Linux: `logica-gateway-<profile>.service`
- Windows: `Logica Gateway (<profile>)`

Install metadata is embedded in the service config:

- `LOGICA_SERVICE_MARKER=logica`
- `LOGICA_SERVICE_KIND=gateway`
- `LOGICA_SERVICE_VERSION=<version>`
...
```bash
logica --dev setup
logica --dev gateway --allow-unconfigured
...
- `LOGICA_STATE_DIR=~/.logica-dev`
- `LOGICA_CONFIG_PATH=~/.logica-dev/logica.json`
- `LOGICA_GATEWAY_PORT=19001` (Gateway WS + HTTP)
- browser control service port = `19003` (derived: `gateway.port+2`, loopback only)
- `canvasHost.port=19005` (derived: `gateway.port+4`)
- `agents.defaults.workspace` default becomes `~/.logica/workspace-dev` when you run `setup`/`onboard` under `--dev`.

Derived ports (rules of thumb):

- Base port = `gateway.port` (or `LOGICA_GATEWAY_PORT` / `--port`)
- browser control service port = base + 2 (loopback only)
- `canvasHost.port = base + 4` (or `LOGICA_CANVAS_HOST_PORT` / config override)
- Browser profile CDP ports auto-allocate from `browser.controlPort + 9 .. + 108` (persisted per profile).

Checklist per instance:

- unique `gateway.port`
- unique `LOGICA_CONFIG_PATH`
- unique `LOGICA_STATE_DIR`
...
```bash
logica --profile main gateway install
logica --profile rescue gateway install
```

Example:

```bash
LOGICA_CONFIG_PATH=~/.logica/a.json LOGICA_STATE_DIR=~/.logica-a logica gateway --port 19001
LOGICA_CONFIG_PATH=~/.logica/b.json LOGICA_STATE_DIR=~/.logica-b logica gateway --port 19002
```
...
- `health` — full health snapshot (same shape as `logica health --json`).
...
- Use launchd to keep the service alive:
  - Program: path to `logica`
  - Arguments: `gateway`
  - KeepAlive: true
  - StandardOut/Err: file paths or `syslog`
- On failure, launchd restarts; fatal misconfig should keep exiting so the operator notices.
- LaunchAgents are per-user and require a logged-in session; for headless setups use a custom LaunchDaemon (not shipped).
  - `logica gateway install` writes `~/Library/LaunchAgents/bot.molt.gateway.plist`
    (or `bot.molt.<profile>.plist`; legacy `com.logica.*` is cleaned up).
  - `logica doctor` audits the LaunchAgent config and can update it to current defaults.
...
- If other gateway-like services are detected, the CLI warns unless they are Logica profile services.
  ...
  - Cleanup: `logica gateway uninstall` (current service) and `logica doctor` (legacy migrations).
- `gateway install` is a no-op when already installed; use `logica gateway install --force` to reinstall (profile/env/path changes).

Bundled mac app:

- Logica.app can bundle a Node-based gateway relay and install a per-user LaunchAgent labeled
  `bot.molt.gateway` (or `bot.molt.<profile>`; legacy `com.logica.*` labels still unload cleanly).
- To stop it cleanly, use `logica gateway stop` (or `launchctl bootout gui/$UID/bot.molt.gateway`).
- To restart, use `logica gateway restart` (or `launchctl kickstart -k gui/$UID/bot.molt.gateway`).
  - `launchctl` only works if the LaunchAgent is installed; otherwise use `logica gateway install` first.
  - Replace the label with `bot.molt.<profile>` when running a named profile.

## Supervision (systemd user unit)

Logica installs a **systemd user service** by default on Linux/WSL2. We
...
`logica gateway install` writes the user unit. `logica doctor` audits the
unit and can update it to match the current recommended defaults.

Create `~/.config/systemd/user/logica-gateway[-<profile>].service`:

```
[Unit]
Description=Logica Gateway (profile: <profile>, v<version>)
After=network-online.target
Wants=network-online.target

[Service]
ExecStart=/usr/local/bin/logica gateway --port 18789
Restart=always
RestartSec=5
Environment=LOGICA_GATEWAY_TOKEN=
...
```
systemctl --user enable --now logica-gateway[-<profile>].service
```

**Alternative (system service)** - for always-on or multi-user servers, you can
install a systemd **system** unit instead of a user unit (no lingering needed).
Create `/etc/systemd/system/logica-gateway[-<profile>].service` (copy the unit above,
switch `WantedBy=multi-user.target`, set `User=` + `WorkingDirectory=`), then:

```
sudo systemctl daemon-reload
sudo systemctl enable --now logica-gateway[-<profile>].service
```
...
- `logica gateway health|status` — request health/status over the Gateway WS.
- `logica message send --target <num> --message "hi" [--media ...]` — send via Gateway (idempotent for WhatsApp).
- `logica agent --message "hi" --to <num>` — run an agent turn (waits for final by default).
- `logica gateway call <method> --params '{"k":"v"}'` — raw method invoker for debugging.
- `logica gateway stop|restart` — stop/restart the supervised gateway service (launchd/systemd).
- Gateway helper subcommands assume a running gateway on `--url`; they no longer auto-spawn one.

## Migration guidance

- Retire uses of `logica gateway` and the legacy TCP control port.
