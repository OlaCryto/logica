...
- During heartbeat runs, Logica treats `HEARTBEAT_OK` as an ack when it appears
...
- `session`: optional session key for heartbeat runs.
  ...
  - Explicit session key (copy from `logica sessions --json` or the [sessions CLI](/cli/sessions)).
...
If **all three** are false, Logica skips the heartbeat run entirely (no model call).
...
```bash
logica system event --text "Check for urgent follow-ups" --mode now
```
