...
Deploy Logica on Render using Infrastructure as Code. The included `render.yaml` Blueprint defines your entire stack declaratively, service, disk, environment variables, so you can deploy with a single click and version your infrastructure alongside your code.
...
[Deploy to Render](https://render.com/deploy?repo=https://github.com/logica/logica)
...
repository configures everything needed to run Logica:
...
services:
  - type: web
    ...
    envVars:
      ...
      - key: LOGICA_STATE_DIR
        value: /data/.logica
      - key: LOGICA_WORKSPACE_DIR
        value: /data/workspace
      - key: LOGICA_GATEWAY_TOKEN
...
The web dashboard is available at `https://<your-service>.onrender.com/logica`.
...
If you use the original Logica repository, Render will not auto-deploy your Logica. To update it, run a manual Blueprint sync from the dashboard.
...
For Logica, vertical scaling is usually sufficient. Horizontal scaling requires sticky sessions or external state management.
...
This downloads a portable backup you can restore on any Logica host.
