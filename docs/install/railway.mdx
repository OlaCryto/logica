...
Deploy Logica on Railway with a one-click template and finish setup in your browser.
...
- `https://<your-railway-domain>/logica` — Control UI
...
- `LOGICA_STATE_DIR=/data/.logica` (recommended)
- `LOGICA_WORKSPACE_DIR=/data/workspace` (recommended)
- `LOGICA_GATEWAY_TOKEN` (recommended; treat as an admin secret)
...
This exports your Logica state + workspace so you can migrate to another host without losing config or memory.
