...
Logica features that can generate provider usage or paid API calls.
...
- `logica status --usage` and `logica channels list` show provider **usage windows**
...
Logica can pick up credentials from:
...
`logica models scan` can probe OpenRouter models and uses `OPENROUTER_API_KEY` when
probing is enabled.
