...
This document explains how Logica manages sessions end-to-end:
...
Logica is designed around a single **Gateway process** that owns session state.
...
Logica persists sessions in two layers:
...
- Transcripts: `~/.logica/agents/<agentId>/sessions/<sessionId>.jsonl`
  - Telegram topic sessions: `.../<sessionId>-topic-<threadId>.jsonl`

Logica resolves these via `src/config/sessions.ts`.
...
Logica intentionally does **not** “fix up” transcripts; the Gateway uses `SessionManager` to read/write them.
...
These are Pi runtime semantics (Logica consumes the events, but Pi decides when to compact).
...
Logica also enforces a safety floor for embedded runs:

- If `compaction.reserveTokens < reserveTokensFloor`, Logica bumps it.
- Default floor is `20000` tokens.
- Set `agents.defaults.compaction.reserveTokensFloor: 0` to disable the floor.
- If it’s already higher, Logica leaves it alone.
...
Logica supports “silent” turns for background tasks where the user should not see intermediate output.

Convention:

- The assistant starts its output with `NO_REPLY` to indicate “do not deliver a reply to the user”.
- Logica strips/suppresses this in the delivery layer.

As of `2026.1.10`, Logica also suppresses **draft/typing streaming** when a partial chunk begins with `NO_REPLY`, so silent operations don’t leak partial output mid-turn.
...
Logica uses the **pre-threshold flush** approach:
...
Pi also exposes a `session_before_compact` hook in the extension API, but Logica’s
...
- Store vs transcript mismatch? Confirm the Gateway host and the store path from `logica status`.
