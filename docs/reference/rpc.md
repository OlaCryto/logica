...
Logica integrates external CLIs via JSON-RPC. Two patterns are used today.
...
- Logica owns lifecycle when `channels.signal.autoStart=true`.
...
- Logica spawns `imsg rpc` as a child process (legacy iMessage integration).
