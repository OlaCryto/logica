...
- **Ask default:** `on-miss`.
- **Approvals store:** `~/.logica/exec-approvals.json` (JSON, no legacy migration).
- **Runner:** headless system service; UI app hosts a Unix socket for approvals.
- **Node identity:** use existing `nodeId`.
- **Socket auth:** Unix socket + token (cross-platform); split later if needed.
- **Node host state:** `~/.logica/node.json` (node id + pairing token).
...
Path: `~/.logica/exec-approvals.json`
...
```json
{
  "version": 1,
  "socket": {
    "path": "~/.logica/exec-approvals.sock",
...
- Unix socket at `~/.logica/exec-approvals.sock` (0600).
