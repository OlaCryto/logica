...
- Publish as `logica/plugin-sdk` (or export from core under `logica/plugin-sdk`).
- Semver with explicit stability guarantees.

### 2) Plugin Runtime (execution surface, injected)

Scope: everything that touches core runtime behavior.
Accessed via `LogicaPluginApi.runtime` so plugins never import `src/**`.
...
export type PluginRuntime = {
  channel: {
    text: {
      ...
      resolveTextChunkLimit(cfg: LogicaConfig, channel: string, accountId?: string): number;
      hasControlCommand(text: string, cfg: LogicaConfig): boolean;
    };
    reply: {
    ...
    mentions: {
      buildMentionRegexes(cfg: LogicaConfig, agentId?: string): RegExp[];
    ...
    debounce: {
      ...
      resolveInboundDebounceMs(cfg: LogicaConfig, channel: string): number;
    };
    commands: {
  ...
  state: {
    resolveStateDir(cfg: LogicaConfig): string;
  };
};
```
...
- Introduce `logica/plugin-sdk`.
- Add `api.runtime` to `LogicaPluginApi` with the surface above.
...
- Plugins declare a required runtime range (e.g., `logicaRuntime: ">=2026.2.0"`).
