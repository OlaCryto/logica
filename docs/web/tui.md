...
```bash
logica tui --url ws://<host>:<port> --token <gateway-token>
```
...
- Turn delivery on:
  ...
  - or start with `logica tui --deliver`
...
- Check the Gateway logs: `logica logs --follow`.
- Confirm the agent can run: `logica status` and `logica models status`.
...
- No agents in picker: check `logica agents list` and your routing config.
