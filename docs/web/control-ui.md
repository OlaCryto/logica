...
- optional prefix: set `gateway.controlUi.basePath` (e.g. `/logica`)
...
If the page fails to load, start the Gateway first: `logica gateway`.
...
you revoke it with `logica devices revoke --device <id> --role <role>`. See
...
- Config: view/edit `~/.logica/logica.json` (`config.get`, `config.set`)
...
(`tailscale-user-login`) when `gateway.auth.allowTailscale` is `true`. Logica
...
Logica **blocks** Control UI connections without device identity.
