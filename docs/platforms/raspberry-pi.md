---
summary: "Logica on Raspberry Pi (budget self-hosted setup)"
read_when:
  - Setting up Logica on a Raspberry Pi
  - Running Logica on ARM devices
  - Building a cheap always-on personal AI
title: "Raspberry Pi"
---

# Logica on Raspberry Pi

## Goal

Run a persistent, always-on Logica Gateway on a Raspberry Pi for **~$35-80** one-time cost (no monthly fees).
...
Most Logica features work on ARM64, but some external binaries may need ARM builds:
...
sudo systemctl is-enabled logica

# Enable if not
sudo systemctl enable logica

# Start on boot
sudo systemctl start logica
```
...
```bash
# Check logs
journalctl -u logica --no-pager -n 100

# Common fix: rebuild
cd ~/logica  # if using hackable install
npm run build
sudo systemctl restart logica
```
