...
- Same path: run the app from a fixed location (for Logica, `dist/Logica.app`).
...
Workaround: move files into the Logica workspace (`~/.logica/workspace`) if you want to avoid per-folder grants.
