...
service via the external `logica` CLI (no embedded runtime). This gives you
...
- The app installs a per‑user LaunchAgent labeled `bot.molt.gateway`
  (or `bot.molt.<profile>` when using `--profile`/`LOGICA_PROFILE`; legacy `com.logica.*` is supported).
