...
3. Set tokens for Logica and start the gateway.
...
7. Slash Commands → create `/logica` if you use `channels.slack.slashCommand`. If you enable native commands, add one slash command per built-in command (same names as `/help`). Native defaults to off for Slack unless you set `channels.slack.commands.native: true` (global `commands.native` is `"auto"` which leaves Slack off).
...
Logica can use a Slack user token (`xoxp-...`) for read operations (history,
...
- Write operations (send/edit/delete messages, add/remove reactions, pin/unpin,
  ...
  no bot token is available, Logica falls back to the user token.
...
```json
{
  "display_information": {
    "name": "Logica",
    "description": "Slack connector for Logica"
  },
  "features": {
    "bot_user": {
      "display_name": "Logica",
    ...
    "slash_commands": [
      {
        "command": "/logica",
        "description": "Send a message to Logica",
        "should_escape": false
      }
    ]
  },
...
By default, Logica replies in the main channel. Use `channels.slack.replyToMode` to control automatic threading:
...
- If Slack doesn’t provide `channel_type`, Logica infers it from the channel ID prefix (`D`, `C`, `G`) and defaults to `channel` to keep session keys stable.
...
- Approve via: `logica pairing approve slack <code>`.
...
- On startup, Logica resolves channel/user names in allowlists to IDs (when tokens allow)
