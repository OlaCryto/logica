...
read_when:
  - You want Logica to receive DMs via Nostr
...
Nostr is a decentralized protocol for social networking. This channel enables Logica to receive and respond to encrypted direct messages (DMs) via NIP-04.

## Install (on demand)

### Onboarding (recommended)

- The onboarding wizard (`logica onboard`) and `logica channels add` list optional channel plugins.
...
```bash
logica plugins install @logica/nostr
```

Use a local checkout (dev workflows):

```bash
logica plugins install --link <path-to-logica>/extensions/nostr
```
  ...
  "channels": {
    "nostr": {
      ...
      "profile": {
        ...
        "nip05": "logica@example.com",
        "lud16": "logica@example.com"
      }
    }
  }
}
