...
```bash
logica plugins install @logica/msteams
```

Local checkout (when running from a git repo):

```bash
logica plugins install ./extensions/msteams
```

If you choose Teams during configure/onboarding and a git checkout is detected,
Logica will offer the local install path automatically.
...
3. Configure Logica with those credentials.
...
- Talk to Logica via Teams DMs, group chats, or channels.
...
- On startup, Logica resolves team/channel and user allowlist names to IDs (when Graph permissions allow)
...
5. Configure `msteams` in `~/.logica/logica.json` (or env vars) and start the gateway.
6. The gateway listens for Bot Framework webhook traffic on `/api/messages` by default.

## Azure Bot Setup (Prerequisites)

Before configuring Logica, you need to create an Azure Bot resource.
...
2. Fill in the **Basics** tab:
   ...
   | Field              | Value                                                    |
   | ------------------ | -------------------------------------------------------- |
   | **Bot handle**     | Your bot name, e.g., `logica-msteams` (must be unique) |
...
1. **Install the Microsoft Teams plugin**
   - From npm: `logica plugins install @logica/msteams`
   - From a local checkout: `logica plugins install ./extensions/msteams`
...
```json
{
  ...
  "description": { "short": "Logica in Teams", "full": "Logica in Teams" },
...
Logica handles this by returning quickly and sending replies proactively, but very slow responses may still cause issues.
...
By default, Logica only downloads media from Microsoft/Teams hostnames. Override with `channels.msteams.mediaAllowHosts` (use `["*"]` to allow any host).
...
Uploaded files are stored in a `/LogicaShared/` folder in the configured SharePoint site's default document library.

## Polls (Adaptive Cards)

Logica sends Teams polls as Adaptive Cards (there is no native Teams poll API).

- CLI: `logica message poll --channel msteams --target conversation:<id> ...`
- Votes are recorded by the gateway in `~/.logica/msteams-polls.json`.
...
```bash
logica message send --channel msteams \
...
logica message send --channel msteams --target "user:40a1a0ed-..." --message "Hello"

# Send to a user by display name (triggers Graph API lookup)
logica message send --channel msteams --target "user:John Smith" --message "Hello"

# Send to a group chat or channel
logica message send --channel msteams --target "conversation:19:abc...@thread.tacv2" --message "Hello"

# Send an Adaptive Card to a conversation
logica message send --channel msteams --target "conversation:19:abc...@thread.tacv2" \
  --card '{"type":"AdaptiveCard","version":"1.5","body":[{"type":"TextBlock","text":"Hello"}]}'
```
