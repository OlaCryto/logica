...
Matrix is an open, decentralized messaging protocol. Logica connects as a Matrix **user**
...
```bash
logica plugins install @logica/matrix
```

Local checkout (when running from a git repo):

```bash
logica plugins install ./extensions/matrix
```
...
1. Install the Matrix plugin:
   - From npm: `logica plugins install @logica/matrix`
   - From a local checkout: `logica plugins install ./extensions/matrix`
...
3. Get an access token for the bot account:
   ...
   - Or set `channels.matrix.userId` + `channels.matrix.password`: Logica calls the same
     login endpoint, stores the access token in `~/.logica/credentials/matrix/credentials.json`,
...
- On first connection, Logica requests device verification from your other sessions.
- Verify the device in another Matrix client (Element, etc.) to enable key sharing.
- If the crypto module cannot be loaded, E2EE is disabled and encrypted rooms will not decrypt;
  Logica logs a warning.
...
`~/.logica/matrix/accounts/<account>/<homeserver>__<user>/<token-hash>/crypto/`
...
- Approve via:
  - `logica pairing list matrix`
  - `logica pairing approve matrix <CODE>`
...
- On startup, Logica resolves room/user names in allowlists to IDs and logs the mapping; unresolved entries are ignored for allowlist matching.
