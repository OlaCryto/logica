...
Feishu (Lark) is a team chat platform used by companies for messaging and collaboration. This plugin connects Logica to a Feishu/Lark bot using the platform’s WebSocket event subscription so messages can be received without exposing a public webhook URL.
...
```bash
logica plugins install @logica/feishu
```

Local checkout (when running from a git repo):

```bash
logica plugins install ./extensions/feishu
```
...
If you just installed Logica, run the wizard:
...
2. Configuring app credentials in Logica
...
1. You already ran `logica channels add` for Feishu
2. The gateway is running (`logica gateway status`)
...
```bash
logica pairing approve feishu <CODE>
```
...
2. Run `logica logs --follow` and look for `chat_id`
...
2. Run `logica logs --follow` and look for `open_id`
...
| Command                    | Description                   |
| -------------------------- | ----------------------------- |
| `logica gateway status`  | Show gateway status           |
| `logica gateway install` | Install/start gateway service |
| `logica gateway stop`    | Stop gateway service          |
| `logica gateway restart` | Restart gateway service       |
| `logica logs --follow`   | Tail gateway logs             |
...
4. Check logs: `logica logs --follow`
...
5. Ensure the gateway is running: `logica gateway status`
6. Check logs: `logica logs --follow`
  ...
  agents: {
    list: [
      { id: "main" },
      {
        ...
        agentDir: "/home/user/.logica/agents/clawd-fan/agent",
      },
      {
        id: "clawd-xi",
        workspace: "/home/user/clawd-xi",
        agentDir: "/home/user/.logica/agents/clawd-xi/agent",
      },
    ],
  },
  bindings: [
