...
Tlon is a decentralized messenger built on Urbit. Logica connects to your Urbit ship and can
...
```bash
logica plugins install @logica/tlon
```

Local checkout (when running from a git repo):

```bash
logica plugins install ./extensions/tlon
```
...
Use these with `logica message send` or cron delivery:
...
- Thread replies: if the inbound message is in a thread, Logica replies in-thread.
