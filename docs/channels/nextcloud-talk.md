...
```bash
logica plugins install @logica/nextcloud-talk
```

Local checkout (when running from a git repo):

```bash
logica plugins install ./extensions/nextcloud-talk
```
...
2. On your Nextcloud server, create a bot:
   ...
   ```bash
   ./occ talk:bot:install "Logica" "<shared-secret>" "<webhook-url>" --feature reaction
   ```
...
- Approve via:
  - `logica pairing list nextcloud-talk`
  - `logica pairing approve nextcloud-talk <CODE>`
