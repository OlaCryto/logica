...
read_when:
  - Setting up Twitch chat integration for Logica
title: "Twitch"
---

# Twitch (plugin)

Twitch chat support via IRC connection. Logica connects as a Twitch user (bot account) to receive and send messages in channels.
...
```bash
logica plugins install @logica/twitch
```

Local checkout (when running from a git repo):

```bash
logica plugins install ./extensions/twitch
```
...
4. Configure the token:
   - Env: `LOGICA_TWITCH_ACCESS_TOKEN=...` (default account only)
...
```json5
{
  channels: {
    twitch: {
      enabled: true,
      username: "logica", // Bot's Twitch account
      accessToken: "oauth:abc123...", // OAuth Access Token (or use LOGICA_TWITCH_ACCESS_TOKEN env var)
...
```bash
LOGICA_TWITCH_ACCESS_TOKEN=oauth:abc123...
```
