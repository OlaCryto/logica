---
read_when:
  - 你想在 Logica 中使用 MiniMax 模型
  - 你需要 MiniMax 设置指南
summary: 在 Logica 中使用 MiniMax M2.1
title: MiniMax
x-i18n:
...
```bash
logica plugins enable minimax-portal-auth  # 如果已加载则跳过
logica gateway restart  # 如果 Gateway 网关已在运行则重启
...
详情参见 [MiniMax OAuth 插件 README](https://github.com/logica/logica/tree/main/extensions/minimax-portal-auth)。
...
- 使用 `logica models list` 和 `logica models set minimax/MiniMax-M2.1` 切换模型。
...
- 运行 `logica configure` 并选择 **MiniMax M2.1**，或
