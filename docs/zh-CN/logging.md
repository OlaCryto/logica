...
你可以在 `~/.logica/logica.json` 中覆盖此设置：

```json
{
  "logging": {
    "file": "/path/to/logica.log"
  }
}
```
...
```bash
logica channels logs --channel whatsapp
```
...
所有日志配置都在 `~/.logica/logica.json` 的 `logging` 下。

```json
{
  "logging": {
    "level": "info",
    "file": "/tmp/logica/logica-YYYY-MM-DD.log",
    "consoleLevel": "info",
    "consoleStyle": "pretty",
...
- Logica 目前通过 **OTLP/HTTP（protobuf）** 导出。
...
```
LOGICA_DIAGNOSTICS=telegram.http,telegram.payload
```
...
- 你也可以使用 `logica plugins enable diagnostics-otel` 启用插件。
...
- `logica.tokens`（计数器，属性：`logica.token`、`logica.channel`、`logica.provider`、`logica.model`）
- `logica.cost.usd`（计数器，属性：`logica.channel`、`logica.provider`、`logica.model`）
- `logica.run.duration_ms`（直方图，属性：`logica.channel`、`logica.provider`、`logica.model`）
- `logica.context.tokens`（直方图，属性：`logica.context`、`logica.channel`、`logica.provider`、`logica.model`）

消息流：

- `logica.webhook.received`（计数器，属性：`logica.channel`、`logica.webhook`）
- `logica.webhook.error`（计数器，属性：`logica.channel`、`logica.webhook`）
- `logica.webhook.duration_ms`（直方图，属性：`logica.channel`、`logica.webhook`）
- `logica.message.queued`（计数器，属性：`logica.channel`、`logica.source`）
- `logica.message.processed`（计数器，属性：`logica.channel`、`logica.outcome`）
- `logica.message.duration_ms`（直方图，属性：`logica.channel`、`logica.outcome`）

队列 + 会话：

- `logica.queue.lane.enqueue`（计数器，属性：`logica.lane`）
- `logica.queue.lane.dequeue`（计数器，属性：`logica.lane`）
- `logica.queue.depth`（直方图，属性：`logica.lane` 或 `logica.channel=heartbeat`）
- `logica.queue.wait_ms`（直方图，属性：`logica.lane`）
- `logica.session.state`（计数器，属性：`logica.state`、`logica.reason`）
- `logica.session.stuck`（计数器，属性：`logica.state`）
- `logica.session.stuck_age_ms`（直方图，属性：`logica.state`）
- `logica.run.attempt`（计数器，属性：`logica.attempt`）

### 导出的 span（名称 + 关键属性）

- `logica.model.usage`
  - `logica.channel`、`logica.provider`、`logica.model`
  - `logica.sessionKey`、`logica.sessionId`
  - `logica.tokens.*`（input/output/cache_read/cache_write/total）
- `logica.webhook.processed`
  - `logica.channel`、`logica.webhook`、`logica.chatId`
- `logica.webhook.error`
  - `logica.channel`、`logica.webhook`、`logica.chatId`、`logica.error`
- `logica.message.processed`
  - `logica.channel`、`logica.outcome`、`logica.chatId`、`logica.messageId`、`logica.sessionKey`、`logica.sessionId`、`logica.reason`
- `logica.session.stuck`
  - `logica.state`、`logica.ageMs`、`logica.queueDepth`、`logica.sessionKey`、`logica.sessionId`
...
- **Gateway 网关无法访问？** 先运行 `logica doctor`。
