...
LINE 通过 LINE Messaging API 连接到 Logica。该插件作为 webhook 接收器在 Gateway 网关上运行，使用你的 channel access token + channel secret 进行身份验证。
...
```bash
logica plugins install @logica/line
```

本地检出（从 git 仓库运行时）：

```bash
logica plugins install ./extensions/line
```
...
```bash
logica pairing list line
logica pairing approve line <CODE>
```
