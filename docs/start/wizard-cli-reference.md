...
read_when:
  - You need detailed behavior for logica onboard
...
This page is the full reference for `logica onboard`.
...
<Steps>
  <Step title="Existing config detection">
    - If `~/.logica/logica.json` exists, choose Keep, Modify, or Reset.
    - Re-running the wizard does not wipe anything unless you explicitly choose Reset (or pass `--reset`).
    - If config is invalid or contains legacy keys, the wizard stops and asks you to run `logica doctor` before continuing.
  ...
  <Step title="Channels">
    ...
    - DM security: default is pairing. First DM sends a code; approve via
      `logica pairing approve <channel> <code>` or use allowlists.
  </Step>
  ...
  <Step title="Health check">
    - Starts gateway (if needed) and runs `logica health`.
    - `logica status --deep` adds gateway health probes to status output.
  </Step>
  <Step title="Skills">
...
<AccordionGroup>
  ...
  <Accordion title="OpenAI API key">
    ...
    `~/.logica/.env` so launchd can read it.
...
- OAuth credentials: `~/.logica/credentials/oauth.json`
- Auth profiles (API keys + OAuth): `~/.logica/agents/<agentId>/agent/auth-profiles.json`
...
- Command reference: [`logica onboard`](/cli/onboard)
