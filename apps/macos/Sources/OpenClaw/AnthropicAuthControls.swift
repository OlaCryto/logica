...
struct AnthropicAuthControls: View {
    ...
    @State private var oauthStatus: LogicaOAuthStore.AnthropicOAuthStatus = LogicaOAuthStore.anthropicOAuthStatus()
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 10) {
            ...
            HStack(spacing: 10) {
                ...
                .disabled(!FileManager().fileExists(atPath: LogicaOAuthStore.oauthURL().path))
    ...
    private func refresh() {
        let imported = LogicaOAuthStore.importLegacyAnthropicOAuthIfNeeded()
        self.oauthStatus = LogicaOAuthStore.anthropicOAuthStatus()
        if imported != nil {
    ...
    private func finishOAuth() async {
        ...
        do {
            ...
            try LogicaOAuthStore.saveAnthropicOAuth(creds)
            self.refresh()
            self.pkce = nil
            self.code = ""
            self.statusText = "Connected. Logica can now use Claude via OAuth."
        } catch {
...
extension AnthropicAuthControls {
    init(
        ...
        oauthStatus: LogicaOAuthStore.AnthropicOAuthStatus,
