...
enum AnthropicAuthMode: Equatable {
    ...
    var shortLabel: String {
        switch self {
        case .oauthFile: "OAuth (Logica token file)"
...
enum AnthropicAuthResolver {
    static func resolve(
        ...
        oauthStatus: LogicaOAuthStore.AnthropicOAuthStatus = LogicaOAuthStore
...
enum AnthropicOAuth {
    private static let logger = Logger(subsystem: "ai.logica", category: "anthropic-oauth")
...
enum LogicaOAuthStore {
    static let oauthFilename = "oauth.json"
    private static let providerKey = "anthropic"
    private static let logicaOAuthDirEnv = "LOGICA_OAUTH_DIR"
    ...
    enum AnthropicOAuthStatus: Equatable {
        ...
        var shortDescription: String {
            switch self {
            case .missingFile: "Logica OAuth token file not found"
            case .unreadableFile: "Logica OAuth token file not readable"
            case .invalidJSON: "Logica OAuth token file invalid"
            case .missingProviderEntry: "No Anthropic entry in Logica OAuth token file"
            case .missingTokens: "Anthropic entry missing tokens"
            case .connected: "Logica OAuth credentials found"
            }
        }
    }

    static func oauthDir() -> URL {
        if let override = ProcessInfo.processInfo.environment[self.logicaOAuthDirEnv]?
