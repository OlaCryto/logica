...
final class PeekabooBridgeHostCoordinator {
    ...
    private let logger = Logger(subsystem: "ai.logica", category: "PeekabooBridge")

    private var host: PeekabooBridgeHost?
    private var services: LogicaPeekabooBridgeServices?
    private static var logicaSocketPath: String {
        let fileManager = FileManager.default
        let base = fileManager.urls(for: .applicationSupportDirectory, in: .userDomainMask).first
            ?? fileManager.homeDirectoryForCurrentUser.appendingPathComponent("Library/Application Support")
        let directory = base.appendingPathComponent("Logica", isDirectory: true)
    ...
    private func startIfNeeded() async {
        ...
        let services = LogicaPeekabooBridgeServices()
        ...
        let host = PeekabooBridgeHost(
            socketPath: Self.logicaSocketPath,
            server: server,
        ...
        await host.start()
        self.logger
            .info("PeekabooBridge host started at \(Self.logicaSocketPath, privacy: .public)")
    }
...
@MainActor
private final class LogicaPeekabooBridgeServices: PeekabooBridgeServiceProviding {
    ...
    init() {
        let logging = LoggingService(subsystem: "ai.logica.peekaboo")
