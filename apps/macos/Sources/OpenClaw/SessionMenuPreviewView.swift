...
enum SessionMenuPreviewLoader {
    private static let logger = Logger(subsystem: "ai.logica", category: "SessionPreview")
    ...
    private static func requestPreview(
        keys: [String],
        maxItems: Int) async throws -> LogicaSessionsPreviewPayload
    {
    ...
    private static func snapshot(
        from entry: LogicaSessionPreviewEntry,
    ...
    private static func cache(payload: LogicaSessionsPreviewPayload, maxItems: Int) async {
    ...
    private static func previewItems(
        from payload: LogicaChatHistoryPayload,
        maxItems: Int) -> [SessionPreviewItem]
    {
        let boundedItems = self.normalizeMaxItems(maxItems)
        let raw: [LogicaKit.AnyCodable] = payload.messages ?? []
    ...
    private static func decodeMessages(_ raw: [LogicaKit.AnyCodable]) -> [LogicaChatMessage] {
        raw.compactMap { item in
            guard let data = try? JSONEncoder().encode(item) else { return nil }
            return try? JSONDecoder().decode(LogicaChatMessage.self, from: data)
        }
    }
    ...
    private static func previewText(for message: LogicaChatMessage) -> String? {
    ...
    private static func isToolCall(_ message: LogicaChatMessage) -> Bool {
    ...
    private static func toolNames(for message: LogicaChatMessage) -> [String] {
        var names: [String] = []
    ...
    private static func mediaSummary(for message: LogicaChatMessage) -> String? {
