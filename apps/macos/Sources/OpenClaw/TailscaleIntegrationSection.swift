...
struct TailscaleIntegrationSection: View {
    ...
    private var authFields: some View {
        ...
        Text("Stored in ~/.logica/logica.json. Prefer LOGICA_GATEWAY_PASSWORD for production.")
            .font(.caption)
            .foregroundStyle(.secondary)
    ...
    private func applySettings() async {
        ...
        if self.connectionMode == .local, !self.isPaused {
            self.statusMessage = "Saved to ~/.logica/logica.json. Restarting gateway…"
        } else {
            self.statusMessage = "Saved to ~/.logica/logica.json. Restart the gateway to apply."
        }
