...
extension OnboardingView {
    ...
    func refreshLocalGatewayProbe() async {
        ...
        await MainActor.run {
            ...
            let expectedTokens = ["node", "logica", "tsx", "pnpm", "bun"]
    ...
    func refreshAnthropicOAuthStatus() {
        _ = LogicaOAuthStore.importLegacyAnthropicOAuthIfNeeded()
        let previous = self.anthropicAuthDetectedStatus
        let status = LogicaOAuthStore.anthropicOAuthStatus()
    ...
    func verifyAnthropicOAuthIfNeeded(force: Bool = false) async {
        ...
        guard let refresh = LogicaOAuthStore.loadAnthropicOAuthRefreshToken(), !refresh.isEmpty else {
        ...
        do {
            ...
            try LogicaOAuthStore.saveAnthropicOAuth(updated)
            self.refreshAnthropicOAuthStatus()
