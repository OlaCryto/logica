...
enum CommandResolver {
    private static let projectRootDefaultsKey = "logica.gatewayProjectRootPath"
    private static let helperName = "logica"

    static func gatewayEntrypoint(in root: URL) -> String? {
        let distEntry = root.appendingPathComponent("dist/index.js").path
        if FileManager().isReadableFile(atPath: distEntry) { return distEntry }
        let logicaEntry = root.appendingPathComponent("logica.mjs").path
        if FileManager().isReadableFile(atPath: logicaEntry) { return logicaEntry }
        let binEntry = root.appendingPathComponent("bin/logica.js").path
    ...
    static func projectRoot() -> URL {
        ...
        let fallback = FileManager().homeDirectoryForCurrentUser
            .appendingPathComponent("Projects/logica")
    ...
    static func preferredPaths(home: URL, current: [String], projectRoot: URL) -> [String] {
        ...
        #endif
        let logicaPaths = self.logicaManagedPaths(home: home)
        if !logicaPaths.isEmpty {
            extras.insert(contentsOf: logicaPaths, at: 1)
        }
        extras.insert(contentsOf: self.nodeManagerBinPaths(home: home), at: 1 + logicaPaths.count)
        var seen = Set<String>()
        // Preserve order while stripping duplicates so PATH lookups remain deterministic.
        return (extras + current).filter { seen.insert($0).inserted }
    }

    private static func logicaManagedPaths(home: URL) -> [String] {
        let bases = [
            home.appendingPathComponent(".logica"),
        ]
        var paths: [String] = []
        for base in bases {
    ...
    static func logicaExecutable(searchPaths: [String]? = nil) -> String? {
        self.findExecutable(named: self.helperName, searchPaths: searchPaths)
    }

    static func projectLogicaExecutable(projectRoot: URL? = nil) -> String? {
        #if DEBUG
    ...
    static func nodeCliPath() -> String? {
        ...
        let candidates = [
            root.appendingPathComponent("logica.mjs").path,
            root.appendingPathComponent("bin/logica.js").path,
        ]
    ...
    static func hasAnyLogicaInvoker(searchPaths: [String]? = nil) -> Bool {
        if self.logicaExecutable(searchPaths: searchPaths) != nil { return true }
        ...
        searchPaths: [String]? = nil) -> [String]
    {
        ...
        case let .success(runtime):
            let root = self.projectRoot()
            if let logicaPath = self.projectLogicaExecutable(projectRoot: root) {
                return [logicaPath, subcommand] + extraArgs
            }
            ...
            if let pnpm = self.findExecutable(named: "pnpm", searchPaths: searchPaths) {
                ...
                return [pnpm, "--silent", "logica", subcommand] + extraArgs
            }
            if let logicaPath = self.logicaExecutable(searchPaths: searchPaths) {
                return [logicaPath, subcommand] + extraArgs
            }

            let missingEntry = """
            logica entrypoint missing (looked for dist/index.js or logica.mjs); run pnpm build.
            """
    ...
    private static func sshNodeCommand(subcommand: String, extraArgs: [String], settings: RemoteSettings) -> [String]? {
        ...
        // Run the real logica CLI on the remote host.
        let exportedPath = [
            "/opt/homebrew/bin",
            "/usr/local/bin",
            "/usr/bin",
        ...
        let projectSection = if userPRJ.isEmpty {
            """
            DEFAULT_PRJ="$HOME/Projects/logica"
        ...
        \(cliSection)
        \(projectSection)
        if command -v logica >/dev/null 2>&1; then
          CLI="$(command -v logica)"
          logica \(quotedArgs);
        ...
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/logica.mjs" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/logica.mjs"
            node "$PRJ/logica.mjs" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/bin/logica.js" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/bin/logica.js"
            node "$PRJ/bin/logica.js" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif command -v pnpm >/dev/null 2>&1; then
          CLI="pnpm --silent logica"
          pnpm --silent logica \(quotedArgs);
        else
          echo "logica CLI missing on remote host"; exit 127;
        fi
        """
        let options: [String] = [
            "-o", "BatchMode=yes",
    ...
    {
        let root = configRoot ?? LogicaConfigFile.loadDict()
