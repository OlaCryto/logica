...
struct ControlAgentEvent: Codable, Sendable, Identifiable {
    ...
    let stream: String
    let ts: Double
    let data: [String: LogicaProtocol.AnyCodable]
    let summary: String?
}
...
final class ControlChannel {
    ...
    private let logger = Logger(subsystem: "ai.logica", category: "control")
    ...
    {
        do {
            let rawParams = params?.reduce(into: [String: LogicaKit.AnyCodable]()) {
                $0[$1.key] = LogicaKit.AnyCodable($1.value.base)
            }
    ...
    private static func bridgeToProtocolArgs(
        _ value: LogicaProtocol.AnyCodable?) -> [String: LogicaProtocol.AnyCodable]?
    {
        guard let value else { return nil }
        if let dict = value.value as? [String: LogicaProtocol.AnyCodable] {
            return dict
        }
        if let dict = value.value as? [String: LogicaKit.AnyCodable],
           let data = try? JSONEncoder().encode(dict),
           let decoded = try? JSONDecoder().decode([String: LogicaProtocol.AnyCodable].self, from: data)
        {
            return decoded
        }
...
extension Notification.Name {
    static let controlHeartbeat = Notification.Name("logica.control.heartbeat")
    static let controlAgentEvent = Notification.Name("logica.control.agent")
}
