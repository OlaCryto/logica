...
enum LogLocator {
    private static var logDir: URL {
        if let override = ProcessInfo.processInfo.environment["LOGICA_LOG_DIR"],
           !override.isEmpty
        {
            return URL(fileURLWithPath: override)
        }
        let preferred = URL(fileURLWithPath: "/tmp/logica")
        return preferred
    }

    private static var stdoutLog: URL {
        logDir.appendingPathComponent("logica-stdout.log")
    }

    private static var gatewayLog: URL {
        logDir.appendingPathComponent("logica-gateway.log")
    }
    ...
    /// Returns the newest log file under /tmp/logica/ (rolling or stdout), or nil if none exist.
