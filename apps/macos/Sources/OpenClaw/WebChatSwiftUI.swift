...
private let webChatSwiftLogger = Logger(subsystem: "ai.logica", category: "WebChatSwiftUI")
...
struct MacGatewayChatTransport: LogicaChatTransport, Sendable {
    ...
    func listSessions(limit: Int?) async throws -> LogicaChatSessionsListResponse {
        ...
        let data = try await GatewayConnection.shared.request(
            ...
            params: params,
            timeoutMs: 15000)
        return try JSONDecoder().decode(LogicaChatSessionsListResponse.self, from: data)
    }
    ...
    static func mapPushToTransportEvent(_ push: GatewayPush) -> LogicaChatTransportEvent? {
        switch push {
        case let .snapshot(hello):
            let ok = (try? JSONDecoder().decode(
                LogicaGatewayHealthOK.self,
...
final class WebChatSwiftUIWindowController {
    ...
    private let hosting: NSHostingController<LogicaChatView>
    ...
    init(sessionKey: String, presentation: WebChatPresentation, transport: any LogicaChatTransport) {
        self.sessionKey = sessionKey
        self.presentation = presentation
        let vm = LogicaChatViewModel(sessionKey: sessionKey, transport: transport)
        let accent = Self.color(fromHex: AppStateStore.shared.seamColorHex)
        self.hosting = NSHostingController(rootView: LogicaChatView(
            viewModel: vm,
    ...
    private static func makeContentController(
        for presentation: WebChatPresentation,
        hosting: NSHostingController<LogicaChatView>) -> NSViewController
    {
