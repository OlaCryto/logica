...
enum DebugActions {
    private static let verboseDefaultsKey = "logica.debug.verboseMain"
    private static let sessionMenuLimit = 12
    private static let onboardingSeenKey = "logica.onboardingSeen"
    ...
    static func openConfigFolder() {
        let url = LogicaPaths.stateDirURL
        NSWorkspace.shared.activateFileViewerSelecting([url])
    }
    ...
    static func sendTestNotification() async {
        _ = await NotificationManager().send(title: "Logica", body: "Test notification", sound: nil)
    }
    ...
    private static func resolveSessionStorePath() -> String {
        ...
        let configURL = LogicaPaths.configURL
        guard
