...
extension ProcessInfo {
    ...
    /// Nix deployments may write defaults into a stable suite (`ai.logica.mac`) even if the shipped
    ...
    static func resolveNixMode(
        ...
        if environment["LOGICA_NIX_MODE"] == "1" { return true }
        if standard.bool(forKey: "logica.nixMode") { return true }
        ...
        if isAppBundle, let stableSuite, stableSuite.bool(forKey: "logica.nixMode") { return true }
