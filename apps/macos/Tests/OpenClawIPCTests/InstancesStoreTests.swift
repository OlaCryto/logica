...
@Suite struct InstancesStoreTests {
    ...
    func presenceEventPayloadDecodesViaJSONEncoder() {
        ...
        let entry: [String: LogicaProtocol.AnyCodable] = [
            "host": .init("gw"),
            "ip": .init("10.0.0.1"),
            "version": .init("2.0.0"),
            "mode": .init("gateway"),
        ...
        let payloadMap: [String: LogicaProtocol.AnyCodable] = [
            "presence": .init([LogicaProtocol.AnyCodable(entry)]),
        ]
        let payload = LogicaProtocol.AnyCodable(payloadMap)
