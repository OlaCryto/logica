...
This is an **alpha** iOS app that connects to an Logica Gateway as a `role: node`.
...
3. On iOS: Logica → Settings → Gateway → paste setup code → Connect
...
cd apps/ios
xcodegen generate
xcodebuild test -project Logica.xcodeproj -scheme Logica -destination "platform=iOS Simulator,name=iPhone 17"
```

## Shared Code

- `apps/shared/LogicaKit` contains the shared transport/types used by the iOS app.
