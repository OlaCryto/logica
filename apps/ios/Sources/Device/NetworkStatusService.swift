...
final class NetworkStatusService: @unchecked Sendable {
    func currentStatus(timeoutMs: Int = 1500) async -> LogicaNetworkStatusPayload {
    ...
    private static func payload(from path: NWPath) -> LogicaNetworkStatusPayload {
        let status: LogicaNetworkPathStatus = switch path.status {
    ...
    private static func fallbackPayload() -> LogicaNetworkStatusPayload {
        LogicaNetworkStatusPayload(
            status: .unsatisfied,
            isExpensive: false,
            isConstrained: false,
            interfaces: [.other])
