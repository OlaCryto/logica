...
final class GatewayConnectionController {
    ...
    private func currentCaps() -> [String] {
        var caps = [LogicaCapability.canvas.rawValue, LogicaCapability.screen.rawValue]
        ...
        if cameraEnabled { caps.append(LogicaCapability.camera.rawValue) }

        let voiceWakeEnabled = UserDefaults.standard.bool(forKey: VoiceWakePreferences.enabledKey)
        if voiceWakeEnabled { caps.append(LogicaCapability.voiceWake.rawValue) }

        let locationModeRaw = UserDefaults.standard.string(forKey: "location.enabledMode") ?? "off"
        let locationMode = LogicaLocationMode(rawValue: locationModeRaw) ?? .off
        if locationMode != .off { caps.append(LogicaCapability.location.rawValue) }

        caps.append(LogicaCapability.device.rawValue)
        caps.append(LogicaCapability.photos.rawValue)
        caps.append(LogicaCapability.contacts.rawValue)
        caps.append(LogicaCapability.calendar.rawValue)
        caps.append(LogicaCapability.reminders.rawValue)
        if Self.motionAvailable() {
            caps.append(LogicaCapability.motion.rawValue)
        }
    ...
    private func currentCommands() -> [String] {
        var commands: [String] = [
            ...
            LogicaSystemCommand.notify.rawValue,
            LogicaChatCommand.push.rawValue,
        ...
        if caps.contains(LogicaCapability.camera.rawValue) {
            commands.append(LogicaCameraCommand.list.rawValue)
            commands.append(LogicaCameraCommand.snap.rawValue)
            commands.append(LogicaCameraCommand.clip.rawValue)
        }
        if caps.contains(LogicaCapability.location.rawValue) {
            commands.append(LogicaLocationCommand.get.rawValue)
        }
        if caps.contains(LogicaCapability.device.rawValue) {
            commands.append(LogicaDeviceCommand.status.rawValue)
            commands.append(LogicaDeviceCommand.info.rawValue)
        }
        if caps.contains(LogicaCapability.photos.rawValue) {
            commands.append(LogicaPhotosCommand.latest.rawValue)
        }
        if caps.contains(LogicaCapability.contacts.rawValue) {
            commands.append(LogicaContactsCommand.search.rawValue)
            commands.append(LogicaContactsCommand.add.rawValue)
        }
        if caps.contains(LogicaCapability.calendar.rawValue) {
            commands.append(LogicaCalendarCommand.events.rawValue)
            commands.append(LogicaCalendarCommand.add.rawValue)
        }
        if caps.contains(LogicaCapability.reminders.rawValue) {
            commands.append(LogicaRemindersCommand.list.rawValue)
            commands.append(LogicaRemindersCommand.add.rawValue)
        }
        if caps.contains(LogicaCapability.motion.rawValue) {
            commands.append(LogicaMotionCommand.activity.rawValue)
            commands.append(LogicaMotionCommand.pedometer.rawValue)
        }
