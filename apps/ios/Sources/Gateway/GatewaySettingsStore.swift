...
enum GatewaySettingsStore {
    private static let gatewayService = "ai.logica.gateway"
    private static let nodeService = "ai.logica.node"
...
enum GatewayDiagnostics {
    private static let logger = Logger(subsystem: "ai.logica.ios", category: "GatewayDiag")
    private static let queue = DispatchQueue(label: "ai.logica.gateway.diagnostics")
    ...
    private static var fileURL: URL? {
        FileManager.default.urls(for: .documentDirectory, in: .userDomainMask).first?
            .appendingPathComponent("logica-gateway.log")
    }
