...
final class MotionService: MotionServicing {
    func activities(params: LogicaMotionActivityParams) async throws -> LogicaMotionActivityPayload {
        ...
        let mapped = try await withCheckedThrowingContinuation { (cont: CheckedContinuation<[LogicaMotionActivityEntry], Error>) in
        ...
        return LogicaMotionActivityPayload(activities: mapped)
    }
    ...
    func pedometer(params: LogicaPedometerParams) async throws -> LogicaPedometerPayload {
        ...
        let payload = try await withCheckedThrowingContinuation { (cont: CheckedContinuation<LogicaPedometerPayload, Error>) in
            pedometer.queryPedometerData(from: start, to: end) { data, error in
                if let error {
                    cont.resume(throwing: error)
                } else {
                    let formatter = ISO8601DateFormatter()
                    let payload = LogicaPedometerPayload(
