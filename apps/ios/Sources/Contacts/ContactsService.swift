...
final class ContactsService: ContactsServicing {
    ...
    func search(params: LogicaContactsSearchParams) async throws -> LogicaContactsSearchPayload {
        ...
        return LogicaContactsSearchPayload(contacts: payload)
    }
    ...
    func add(params: LogicaContactsAddParams) async throws -> LogicaContactsAddPayload {
        ...
        if !phoneNumbers.isEmpty || !emails.isEmpty {
            if let existing = try Self.findExistingContact(
                ...
                emails: emails)
            {
                return LogicaContactsAddPayload(contact: Self.payload(from: existing))
            }
        }
        ...
        return LogicaContactsAddPayload(contact: Self.payload(from: persisted))
    }
    ...
    private static func payload(from contact: CNContact) -> LogicaContactPayload {
        LogicaContactPayload(
