...
final class RemindersService: RemindersServicing {
    func list(params: LogicaRemindersListParams) async throws -> LogicaRemindersListPayload {
        ...
        let payload = try await withCheckedThrowingContinuation { (cont: CheckedContinuation<[LogicaReminderPayload], Error>) in
        ...
        return LogicaRemindersListPayload(reminders: payload)
    }
    ...
    func add(params: LogicaRemindersAddParams) async throws -> LogicaRemindersAddPayload {
        ...
        let formatter = ISO8601DateFormatter()
        let due = reminder.dueDateComponents.flatMap { Calendar.current.date(from: $0) }
        let payload = LogicaReminderPayload(
            identifier: reminder.calendarItemIdentifier,
            title: reminder.title,
            dueISO: due.map { formatter.string(from: $0) },
            completed: reminder.isCompleted,
        ...
        return LogicaRemindersAddPayload(reminder: payload)
    }
