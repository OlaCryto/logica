...
struct IOSGatewayChatTransport: LogicaChatTransport, Sendable {
    ...
    func listSessions(limit: Int?) async throws -> LogicaChatSessionsListResponse {
        ...
        return try JSONDecoder().decode(LogicaChatSessionsListResponse.self, from: res)
    }
    ...
    func requestHistory(sessionKey: String) async throws -> LogicaChatHistoryPayload {
        ...
        return try JSONDecoder().decode(LogicaChatHistoryPayload.self, from: res)
    }
        ...
        attachments: [LogicaChatAttachmentPayload]) async throws -> LogicaChatSendResponse
    {
        struct Params: Codable {
            ...
            var message: String
            var thinking: String
            var attachments: [LogicaChatAttachmentPayload]?
            var timeoutMs: Int
        ...
        return try JSONDecoder().decode(LogicaChatSendResponse.self, from: res)
    }

    func requestHealth(timeoutMs: Int) async throws -> Bool {
        let seconds = max(1, Int(ceil(Double(timeoutMs) / 1000.0)))
        let res = try await self.gateway.request(method: "health", paramsJSON: nil, timeoutSeconds: seconds)
        return (try? JSONDecoder().decode(LogicaGatewayHealthOK.self, from: res))?.ok ?? true
    }
                ...
                for await evt in stream {
                    ...
                    case "health":
                        ...
                        let ok = (try? GatewayPayloadDecoding.decode(
                            payload,
                            as: LogicaGatewayHealthOK.self))?.ok ?? true
                        continuation.yield(.health(ok: ok))
                    case "chat":
                        guard let payload = evt.payload else { break }
                        if let chatPayload = try? GatewayPayloadDecoding.decode(
                            payload,
                            as: LogicaChatEventPayload.self)
                        {
                    ...
                    case "agent":
                        ...
                        if let agentPayload = try? GatewayPayloadDecoding.decode(
                            payload,
                            as: LogicaAgentEventPayload.self)
                        {
