...
private let chatUILogger = Logger(subsystem: "ai.logica", category: "LogicaChatUI")

@MainActor
@Observable
public final class LogicaChatViewModel {
    public private(set) var messages: [LogicaChatMessage] = []
    ...
    public var attachments: [LogicaPendingAttachment] = []
    ...
    public private(set) var pendingToolCalls: [LogicaChatPendingToolCall] = []
    public private(set) var sessions: [LogicaChatSessionEntry] = []
    private let transport: any LogicaChatTransport
    ...
    private var pendingToolCallsById: [String: LogicaChatPendingToolCall] = [:] {
        didSet {
    ...
    public init(sessionKey: String, transport: any LogicaChatTransport) {
    ...
    public var sessionChoices: [LogicaChatSessionEntry] {
        let now = Date().timeIntervalSince1970 * 1000
        let cutoff = now - (24 * 60 * 60 * 1000)
        let sorted = self.sessions.sorted { ($0.updatedAt ?? 0) > ($1.updatedAt ?? 0) }
        var seen = Set<String>()
        var recent: [LogicaChatSessionEntry] = []
        for entry in sorted {
        ...
        var result: [LogicaChatSessionEntry] = []
    ...
    public func removeAttachment(_ id: LogicaPendingAttachment.ID) {
    ...
    private static func decodeMessages(_ raw: [AnyCodable]) -> [LogicaChatMessage] {
        let decoded = raw.compactMap { item in
            (try? ChatPayloadDecoding.decode(item, as: LogicaChatMessage.self))
        }
        return Self.dedupeMessages(decoded)
    }

    private static func dedupeMessages(_ messages: [LogicaChatMessage]) -> [LogicaChatMessage] {
        var result: [LogicaChatMessage] = []
    ...
    private static func dedupeKey(for message: LogicaChatMessage) -> String? {
    ...
    private func performSend() async {
        ...
        var userContent: [LogicaChatMessageContent] = [
        ...
        let encodedAttachments = self.attachments.map { att -> LogicaChatAttachmentPayload in
            LogicaChatAttachmentPayload(
                type: att.type,
    ...
    private func placeholderSession(key: String) -> LogicaChatSessionEntry {
        LogicaChatSessionEntry(
            key: key,
            kind: nil,
            displayName: nil,
            surface: nil,
    ...
    private func handleTransportEvent(_ evt: LogicaChatTransportEvent) {
        switch evt {
        case let .health(ok):
            self.healthOK = ok
        case .tick:
    ...
    private func handleChatEvent(_ chat: LogicaChatEventPayload) {
    ...
    private func handleAgentEvent(_ evt: LogicaAgentEventPayload) {
        ...
        case "tool":
            ...
            if phase == "start" {
                ...
                self.pendingToolCallsById[toolCallId] = LogicaChatPendingToolCall(
    ...
    private static func previewImage(data: Data) -> LogicaPlatformImage? {
        #if canImport(AppKit)
        NSImage(data: data)
        #elseif canImport(UIKit)
        UIImage(data: data)
