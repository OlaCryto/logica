...
@MainActor
struct LogicaChatComposer: View {
    @Bindable var viewModel: LogicaChatViewModel
        ...
        .background {
            ...
            if self.style == .standard {
                let shape = UnevenRoundedRectangle(
                    ...
                    style: .continuous)
                shape
                    .fill(LogicaChatTheme.composerBackground)
                    .overlay(shape.strokeBorder(LogicaChatTheme.composerBorder, lineWidth: 1))
        ...
        ScrollView(.horizontal, showsIndicators: false) {
            HStack(spacing: 6) {
                ForEach(
                    ...
                    id: \LogicaPendingAttachment.id)
                { (att: LogicaPendingAttachment) in
                    HStack(spacing: 6) {
                        if let img = att.preview {
                            LogicaPlatformImageFactory.image(img)
                                .resizable()
                                .scaledToFill()
    ...
    private var editor: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 12, style: .continuous)
                .fill(LogicaChatTheme.composerField)
                .overlay(
                    RoundedRectangle(cornerRadius: 12, style: .continuous)
                        .strokeBorder(LogicaChatTheme.composerBorder)))
    ...
    private var connectionPill: some View {
        ...
        .padding(.horizontal, 8)
        .padding(.vertical, 4)
        .background(LogicaChatTheme.subtleCard)
        .clipShape(Capsule())
    }
