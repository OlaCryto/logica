...
private final class TestChatTransport: @unchecked Sendable, LogicaChatTransport {
    private let state = TestChatTransportState()
    private let historyResponses: [LogicaChatHistoryPayload]
    private let sessionsResponses: [LogicaChatSessionsListResponse]

    private let stream: AsyncStream<LogicaChatTransportEvent>
    private let continuation: AsyncStream<LogicaChatTransportEvent>.Continuation

    init(
        historyResponses: [LogicaChatHistoryPayload],
        sessionsResponses: [LogicaChatSessionsListResponse] = [])
    {
        self.historyResponses = historyResponses
        self.sessionsResponses = sessionsResponses
        var cont: AsyncStream<LogicaChatTransportEvent>.Continuation!
    ...
    func requestHistory(sessionKey: String) async throws -> LogicaChatHistoryPayload {
        ...
        return self.historyResponses.last ?? LogicaChatHistoryPayload(
    ...
    func sendMessage(
        ...
        attachments _: [LogicaChatAttachmentPayload]) async throws -> LogicaChatSendResponse
    {
        await self.state.sentRunIdsAppend(idempotencyKey)
        return LogicaChatSendResponse(runId: idempotencyKey, status: "ok")
    }
    ...
    func listSessions(limit _: Int?) async throws -> LogicaChatSessionsListResponse {
        ...
        return self.sessionsResponses.last ?? LogicaChatSessionsListResponse(
            ts: nil,
            path: nil,
            count: 0,
            defaults: nil,
    ...
    func emit(_ evt: LogicaChatTransportEvent) {
...
@Suite struct ChatViewModelTests {
    @Test func streamsAssistantAndClearsOnFinal() async throws {
        ...
        let history1 = LogicaChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [],
            thinkingLevel: "off")
        let history2 = LogicaChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [
                AnyCodable([
        ...
        let vm = await MainActor.run { LogicaChatViewModel(sessionKey: "main", transport: transport) }
    ...
    @Test func clearsStreamingOnExternalFinalEvent() async throws {
        let sessionId = "sess-main"
        let history = LogicaChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [],
            thinkingLevel: "off")
    ...
    @Test func sessionChoicesPreferMainAndRecent() async throws {
        ...
        let history = LogicaChatHistoryPayload(
            ...
            messages: [],
            thinkingLevel: "off")
        let sessions = LogicaChatSessionsListResponse(
            ts: now,
            path: nil,
            count: 4,
            defaults: nil,
    ...
    @Test func sessionChoicesIncludeCurrentWhenMissing() async throws {
        ...
        let transport = TestChatTransport(
            historyResponses: [history],
            sessionsResponses: [sessions])
        let vm = await MainActor.run { LogicaChatViewModel(sessionKey: "custom", transport: transport) }
        await MainActor.run { vm.load() }
        try await waitUntil("sessions loaded") { await MainActor.run { !vm.sessions.isEmpty } }
