...
function restoreEnv(snapshot: EnvSnapshot) {
  ...
  restoreKey("LOGICA_STATE_DIR", snapshot.stateDir);
}
...
function setTempHome(base: string) {
  ...
  process.env.LOGICA_STATE_DIR = path.join(base, ".logica");
...
export async function withTempHome<T>(
  ...
  const base = await fs.mkdtemp(path.join(os.tmpdir(), opts.prefix ?? "logica-test-home-"));
  ...
  setTempHome(base);
  await fs.mkdir(path.join(base, ".logica", "agents", "main", "sessions"), { recursive: true });
  if (opts.env) {
