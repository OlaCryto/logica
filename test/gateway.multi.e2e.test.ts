...
const spawnGatewayInstance = async (name: string): Promise<GatewayInstance> => {
  ...
  const homeDir = await fs.mkdtemp(path.join(os.tmpdir(), `logica-e2e-${name}-`));
  const configDir = path.join(homeDir, ".logica");
    ...
    child = spawn(
      ...
      ],
      {
        ...
        env: {
          ...
          LOGICA_SKIP_BROWSER_CONTROL_SERVER: "1",
...
const waitForNodeStatus = async (inst: GatewayInstance, nodeId: string, timeoutMs = 10_000) => {
  ...
  while (Date.now() < deadline) {
    const list = (await runCliJson(
      ...
      {
        LOGICA_GATEWAY_TOKEN: inst.gatewayToken,
        LOGICA_GATEWAY_PASSWORD: "",
      },
    )) as NodeListPayload;
    ...
    async () => {
      ...
      const [healthA, healthB] = (await Promise.all([
        runCliJson(["health", "--json", "--timeout", "10000"], {
          LOGICA_GATEWAY_PORT: String(gwA.port),
          LOGICA_GATEWAY_TOKEN: gwA.gatewayToken,
          LOGICA_GATEWAY_PASSWORD: "",
        }),
        runCliJson(["health", "--json", "--timeout", "10000"], {
          LOGICA_GATEWAY_PORT: String(gwB.port),
          LOGICA_GATEWAY_TOKEN: gwB.gatewayToken,
          LOGICA_GATEWAY_PASSWORD: "",
        }),
