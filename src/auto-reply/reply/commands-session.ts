...
import { scheduleGatewaySigusr1Restart, triggerLogicaRestart } from "../../infra/restart.js";
...
export const handleRestartCommand: CommandHandler = async (params, allowTextCommands) => {
  ...
  if (hasSigusr1Listener) {
    ...
    return {
      shouldContinue: false,
      reply: {
        text: "⚙️ Restarting Logica in-process (SIGUSR1); back in a few seconds.",
      },
    };
  }
  const restartMethod = triggerLogicaRestart();
  if (!restartMethod.ok) {
  ...
  return {
    shouldContinue: false,
    reply: {
      text: `⚙️ Restarting Logica via ${restartMethod.method}; give me a few seconds to come back online.`,
    },
  };
};
