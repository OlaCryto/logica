...
beforeAll(async () => {
  testWorkspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "logica-commands-"));
...
function buildParams(commandBody: string, cfg: LogicaConfig, ctxOverrides?: Partial<MsgContext>) {
  const ctx = {
    Body: commandBody,
