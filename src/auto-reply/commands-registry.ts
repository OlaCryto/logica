...
import type { LogicaConfig } from "../config/types.js";
import type {
...
export function isCommandEnabled(cfg: LogicaConfig, commandKey: string): boolean {
...
function resolveDefaultCommandContext(cfg?: LogicaConfig): {
  provider: string;
  model: string;
} {
  const resolved = resolveConfiguredModelRef({
    cfg: cfg ?? ({} as LogicaConfig),
...
export function getCommandDetection(_cfg?: LogicaConfig): CommandDetection {
  const commands = getChatCommands();
...
export function maybeResolveTextAlias(raw: string, cfg?: LogicaConfig) {
