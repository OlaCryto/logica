...
describe("resolveEffectiveHomeDir", () => {
  it("prefers LOGICA_HOME over HOME and USERPROFILE", () => {
    const env = {
  ...
  it("expands LOGICA_HOME when set to ~", () => {
    const env = {
      LOGICA_HOME: "~/svc",
      HOME: "/home/alice",
    } as NodeJS.ProcessEnv;
...
describe("resolveRequiredHomeDir", () => {
  ...
  it("returns a fully resolved path for LOGICA_HOME", () => {
    const result = resolveRequiredHomeDir(
      { LOGICA_HOME: "/custom/home" } as NodeJS.ProcessEnv,
      () => "/fallback",
    );
    expect(result).toBe(path.resolve("/custom/home"));
  });

  it("returns cwd when LOGICA_HOME is tilde-only and no fallback home exists", () => {
    expect(
      resolveRequiredHomeDir({ LOGICA_HOME: "~" } as NodeJS.ProcessEnv, () => {
        throw new Error("no home");
      }),
    ).toBe(process.cwd());
  });
...
describe("expandHomePrefix", () => {
  it("expands tilde using effective home", () => {
    const value = expandHomePrefix("~/x", {
      env: { LOGICA_HOME: "/srv/logica-home" } as NodeJS.ProcessEnv,
    });
    expect(value).toBe(`${path.resolve("/srv/logica-home")}/x`);
  });
