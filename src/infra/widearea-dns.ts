...
export function resolveWideAreaDiscoveryDomain(params?: {
  ...
  const candidate = params?.configDomain ?? env.LOGICA_WIDE_AREA_DOMAIN ?? null;
...
function extractContentHash(zoneText: string): string | null {
  const match = zoneText.match(/^\s*;\s*logica-content-hash:\s*(\S+)\s*$/m);
  return match?.[1] ?? null;
}
...
function renderZone(opts: WideAreaGatewayZoneOpts & { serial: number }): string {
  const hostname = os.hostname().split(".")[0] ?? "logica";
  const hostLabel = dnsLabel(opts.hostLabel ?? hostname, "logica");
  const instanceLabel = dnsLabel(opts.instanceLabel ?? `${hostname}-gateway`, "logica-gw");
  ...
  records.push(`_logica-gw._tcp IN PTR ${instanceLabel}._logica-gw._tcp`);
  records.push(`${instanceLabel}._logica-gw._tcp IN SRV 0 0 ${opts.gatewayPort} ${hostLabel}`);
  records.push(`${instanceLabel}._logica-gw._tcp IN TXT ${txt.map(txtQuote).join(" ")}`);
  ...
  return `; logica-content-hash: ${contentHash}\n${contentBody}`;
}
