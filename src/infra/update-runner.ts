...
export async function runGatewayUpdate(opts: UpdateRunnerOptions = {}): Promise<UpdateRunResult> {
  ...
  if (gitRoot && pkgRoot && path.resolve(gitRoot) === path.resolve(pkgRoot)) {
    ...
    if (channel === "dev") {
      ...
      const preflightRoot = await fs.mkdtemp(path.join(os.tmpdir(), "logica-update-preflight-"));
    ...
    const doctorEntry = path.join(gitRoot, "logica.mjs");
    ...
    const doctorStep = await runStep(
      step("logica doctor", doctorArgv, gitRoot, { LOGICA_UPDATE_IN_PROGRESS: "1" }),
    );
