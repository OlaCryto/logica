...
export function triggerLogicaRestart(): RestartAttempt {
  ...
  if (process.platform !== "darwin") {
    if (process.platform === "linux") {
      const unit = normalizeSystemdUnit(
        process.env.LOGICA_SYSTEMD_UNIT,
        process.env.LOGICA_PROFILE,
      );
  ...
  const label =
    process.env.LOGICA_LAUNCHD_LABEL ||
    resolveGatewayLaunchAgentLabel(process.env.LOGICA_PROFILE);
