...
function isDisabledByEnv() {
  if (isTruthyEnvValue(process.env.LOGICA_DISABLE_BONJOUR)) {
    return true;
  }
...
function safeServiceName(name: string) {
  ...
  return trimmed.length > 0 ? trimmed : "Logica";
}

function prettifyInstanceName(name: string) {
  const normalized = name.trim().replace(/\s+/g, " ");
  return normalized.replace(/\s+\(Logica\)\s*$/i, "").trim() || normalized;
}
...
export async function startGatewayBonjourAdvertiser(
  ...
  const hostnameRaw =
    process.env.LOGICA_MDNS_HOSTNAME?.trim() ||
    process.env.LOGICA_MDNS_HOSTNAME?.trim() ||
    "logica";
  const hostname =
    hostnameRaw
