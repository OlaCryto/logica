...
function candidateBinDirs(opts: EnsureLogicaPathOpts): string[] {
  ...
  // Bundled macOS app: `logica` lives next to the executable (process.execPath).
  try {
    const execDir = path.dirname(execPath);
    const siblingCli = path.join(execDir, "logica");
  ...
  // Project-local installs (best effort): if a `node_modules/.bin/logica` exists near cwd,
  // include it. This helps when running under launchd or other minimal PATH environments.
  const localBinDir = path.join(cwd, "node_modules", ".bin");
  if (isExecutable(path.join(localBinDir, "logica"))) {
...
/**
 * Best-effort PATH bootstrap so skills that require the `logica` CLI can run
 * under launchd/minimal environments (and inside the macOS app bundle).
 */
export function ensureLogicaCliOnPath(opts: EnsureLogicaPathOpts = {}) {
  if (isTruthyEnvValue(process.env.LOGICA_PATH_BOOTSTRAPPED)) {
    return;
  }
  process.env.LOGICA_PATH_BOOTSTRAPPED = "1";
