...
const makeConfig = (opts?: { fallbacks?: string[]; apiKey?: string }): LogicaConfig =>
  ({
    agents: {
      defaults: {
        model: {
...
describe("runEmbeddedPiAgent auth profile rotation", () => {
  it("rotates for auto-pinned profiles", async () => {
    const agentDir = await fs.mkdtemp(path.join(os.tmpdir(), "logica-agent-"));
    const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "logica-workspace-"));
    try {
