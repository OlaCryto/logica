...
import { createLogicaCodingTools } from "./pi-tools.js";
...
describe("workspace path resolution", () => {
  it("reads relative paths against workspaceDir even after cwd changes", async () => {
    await withTempDir("logica-ws-", async (workspaceDir) => {
      await withTempDir("logica-cwd-", async (otherDir) => {
        ...
        try {
          const tools = createLogicaCodingTools({ workspaceDir });
    ...
    await withTempDir("logica-ws-", async (workspaceDir) => {
      await withTempDir("logica-cwd-", async (otherDir) => {
        ...
        try {
          ...
          expect(updated).toBe("hello logica");
        } finally {
          cwdSpy.mockRestore();
        }
      });
  ...
  it("defaults exec cwd to workspaceDir when workdir is omitted", async () => {
    await withTempDir("logica-ws-", async (workspaceDir) => {
      const tools = createLogicaCodingTools({ workspaceDir, exec: { host: "gateway" } });
  ...
  it("lets exec workdir override the workspace default", async () => {
    await withTempDir("logica-ws-", async (workspaceDir) => {
      await withTempDir("logica-override-", async (overrideDir) => {
        const tools = createLogicaCodingTools({ workspaceDir, exec: { host: "gateway" } });
        const execTool = tools.find((tool) => tool.name === "exec");
        expect(execTool).toBeDefined();
...
describe("sandboxed workspace paths", () => {
  it("uses sandbox workspace for relative read/write/edit", async () => {
    await withTempDir("logica-sandbox-", async (sandboxDir) => {
      await withTempDir("logica-workspace-", async (workspaceDir) => {
        const sandbox = {
          enabled: true,
        ...
        const tools = createLogicaCodingTools({ workspaceDir, sandbox });
        const readTool = tools.find((tool) => tool.name === "read");
        const writeTool = tools.find((tool) => tool.name === "write");
        const editTool = tools.find((tool) => tool.name === "edit");
