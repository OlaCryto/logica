...
function resolveExecConfig(cfg: LogicaConfig | undefined) {
...
export function createLogicaCodingTools(options?: {
  ...
  const base = (codingTools as unknown as AnyAgentTool[]).flatMap((tool) => {
    if (tool.name === readTool.name) {
      ...
      return [createLogicaReadTool(freshReadTool)];
    }
