...
/**
 * Cleanup suspended Logica CLI processes that have accumulated.
...
function buildModelAliasLines(cfg?: LogicaConfig) {
...
export async function writeCliImages(
  ...
  const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "logica-cli-images-"));
  const paths: string[] = [];
