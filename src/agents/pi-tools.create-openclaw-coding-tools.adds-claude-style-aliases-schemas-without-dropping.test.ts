...
import { __testing, createLogicaCodingTools } from "./pi-tools.js";
...
describe("createLogicaCodingTools", () => {
  ...
  it("does not expose provider-specific message tools", () => {
    const tools = createLogicaCodingTools({ messageProvider: "discord" });
  ...
  it("applies sandbox path guards to file_path alias", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "logica-sbx-"));
    const outsidePath = path.join(os.tmpdir(), "logica-outside.txt");
