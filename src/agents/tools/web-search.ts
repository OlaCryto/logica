...
type WebSearchConfig = NonNullable<LogicaConfig["tools"]>["web"] extends infer Web
...
function resolveSearchConfig(cfg?: LogicaConfig): WebSearchConfig {
...
function missingSearchKeyPayload(provider: (typeof SEARCH_PROVIDERS)[number]) {
  ...
  return {
    ...
    message: `web_search needs a Brave Search API key. Run \`${formatCliCommand("logica configure --section web")}\` to store it, or set BRAVE_API_KEY in the Gateway environment.`,
    docs: "https://docs.logica.ai/tools/web",
  };
}
...
async function runPerplexitySearch(params: {
  ...
  const res = await fetch(endpoint, {
    ...
    headers: {
      ...
      "HTTP-Referer": "https://logica.ai",
      "X-Title": "Logica Web Search",
    },
    body: JSON.stringify({
      model,
      messages: [
