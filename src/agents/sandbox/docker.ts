...
export function buildSandboxCreateArgs(params: {
  ...
  args.push("--label", "logica.sandbox=1");
  args.push("--label", `logica.sessionKey=${params.scopeKey}`);
  args.push("--label", `logica.createdAtMs=${createdAtMs}`);
  if (params.configHash) {
    args.push("--label", `logica.configHash=${params.configHash}`);
  }
...
async function readContainerConfigHash(containerName: string): Promise<string | null> {
  ...
  return await readLabel("logica.configHash");
}

function formatSandboxRecreateHint(params: { scope: SandboxConfig["scope"]; sessionKey: string }) {
  if (params.scope === "session") {
    return formatCliCommand(`logica sandbox recreate --session ${params.sessionKey}`);
  }
  if (params.scope === "agent") {
    const agentId = resolveSandboxAgentId(params.sessionKey) ?? "main";
    return formatCliCommand(`logica sandbox recreate --agent ${agentId}`);
  }
  return formatCliCommand("logica sandbox recreate --all");
}
