...
${formatHelpExamples([
  ['logica agent --to +15555550123 --message "status update"', "Start a new session."],
  ['logica agent --agent ops --message "Summarize logs"', "Use a specific agent."],
  [
    'logica agent --session-id 1234 --message "Summarize inbox" --thinking medium',
    "Target a session with explicit thinking level.",
  ],
  [
    'logica agent --to +15555550123 --message "Trace logs" --verbose on --json',
    "Enable verbose logging and JSON output.",
  ],
  ['logica agent --to +15555550123 --message "Summon reply" --deliver', "Deliver reply."],
  [
    'logica agent --agent ops --message "Generate report" --deliver --reply-channel slack --reply-to "#reports"',
    "Send reply to a different channel/target.",
  ],
])}

${theme.muted("Docs:")} ${formatDocsLink("/cli/agent", "docs.logica.ai/cli/agent")}`,
    )
    .action(async (opts) => {
  ...
  const agents = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/agents", "docs.logica.ai/cli/agents")}\n`,
    );
...
${theme.heading("Examples:")}
${formatHelpExamples([
  ['logica agents set-identity --agent main --name "Logica" --emoji "🦞"', "Set name + emoji."],
  ["logica agents set-identity --agent main --avatar avatars/logica.png", "Set avatar path."],
  [
    "logica agents set-identity --workspace ~/.logica/workspace --from-identity",
    "Load from IDENTITY.md.",
  ],
  [
    "logica agents set-identity --identity-file ~/.logica/workspace/IDENTITY.md --agent main",
