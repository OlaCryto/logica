...
import { resolveLogicaPackageRoot } from "../infra/logica-root.js";
...
const STEP_LABELS: Record<string, string> = {
  ...
  "logica doctor entry": "Checking doctor entrypoint",
  "logica doctor": "Running doctor checks",
...
const DEFAULT_PACKAGE_NAME = "logica";
const CORE_PACKAGE_NAMES = new Set([DEFAULT_PACKAGE_NAME]);
const CLI_NAME = resolveCliName();
const LOGICA_REPO_URL = "https://github.com/logica/logica.git";
...
function normalizeTag(value?: string | null): string | null {
  ...
  if (trimmed.startsWith("logica@")) {
    return trimmed.slice("logica@".length);
  }
...
async function tryWriteCompletionCache(root: string, jsonMode: boolean): Promise<void> {
  const binPath = path.join(root, "logica.mjs");
  ...
  if (!status.profileInstalled) {
    ...
    if (isCancel(shouldInstall) || !shouldInstall) {
      if (!opts.skipPrompt) {
        defaultRuntime.log(
          theme.muted(
            `Skipped. Run \`${replaceCliName(formatCliCommand("logica completion --install"), CLI_NAME)}\` later to enable.`,
          ),
        );
      }
      return;
...
function resolveGitInstallDir(): string {
  const override = process.env.LOGICA_GIT_DIR?.trim();
  if (override) {
...
async function ensureGitCheckout(params: {
  ...
  if (!dirExists) {
    return await runUpdateStep({
      name: "git clone",
      argv: ["git", "clone", LOGICA_REPO_URL, params.dir],
      timeoutMs: params.timeoutMs,
  ...
  if (!(await isGitCheckout(params.dir))) {
    ...
    if (!empty) {
      throw new Error(
        `LOGICA_GIT_DIR points at a non-git directory: ${params.dir}. Set LOGICA_GIT_DIR to an empty folder or an logica checkout.`,
      );
    }
    return await runUpdateStep({
      name: "git clone",
  ...
  if (!(await isCorePackage(params.dir))) {
    throw new Error(`LOGICA_GIT_DIR does not look like a core checkout: ${params.dir}.`);
  }
...
export async function updateStatusCommand(opts: UpdateStatusOptions): Promise<void> {
  ...
  const root =
    (await resolveLogicaPackageRoot({
  ...
  defaultRuntime.log(theme.heading("Logica update status"));
  defaultRuntime.log("");
  defaultRuntime.log(
    renderTable({
      width: tableWidth,
...
export async function updateCommand(opts: UpdateCommandOptions): Promise<void> {
  ...
  if (!opts.json) {
    defaultRuntime.log(theme.heading("Updating Logica..."));
    defaultRuntime.log("");
  }
  ...
  if (result.status === "skipped") {
    ...
    if (result.reason === "not-git-install") {
      defaultRuntime.log(
        theme.warn(
          `Skipped: this Logica install isn't a git checkout, and the package manager couldn't be detected. Update via your package manager, then run \`${replaceCliName(formatCliCommand("logica doctor"), CLI_NAME)}\` and \`${replaceCliName(formatCliCommand("logica gateway restart"), CLI_NAME)}\`.`,
        ),
      );
      defaultRuntime.log(
        theme.muted(
          `Examples: \`${replaceCliName("npm i -g logica@latest", CLI_NAME)}\` or \`${replaceCliName("pnpm add -g logica@latest", CLI_NAME)}\``,
        ),
      );
    }
    defaultRuntime.exit(0);
  ...
  if (shouldRestart) {
    ...
    try {
      ...
      if (!opts.json && restarted) {
        ...
        defaultRuntime.log("");
        process.env.LOGICA_UPDATE_IN_PROGRESS = "1";
        try {
          ...
          delete process.env.LOGICA_UPDATE_IN_PROGRESS;
        }
      }
    } catch (err) {
      if (!opts.json) {
        ...
        defaultRuntime.log(
          theme.muted(
            `You may need to restart the service manually: ${replaceCliName(formatCliCommand("logica gateway restart"), CLI_NAME)}`,
          ),
        );
      }
    }
    ...
    if (result.mode === "npm" || result.mode === "pnpm") {
      defaultRuntime.log(
        theme.muted(
          `Tip: Run \`${replaceCliName(formatCliCommand("logica doctor"), CLI_NAME)}\`, then \`${replaceCliName(formatCliCommand("logica gateway restart"), CLI_NAME)}\` to apply updates to a running gateway.`,
        ),
      );
    } else {
      defaultRuntime.log(
        theme.muted(
          `Tip: Run \`${replaceCliName(formatCliCommand("logica gateway restart"), CLI_NAME)}\` to apply updates to a running gateway.`,
        ),
      );
    }
  }
...
export async function updateWizardCommand(opts: UpdateWizardOptions = {}): Promise<void> {
  if (!process.stdin.isTTY) {
    defaultRuntime.error(
      "Update wizard requires a TTY. Use `logica update --channel <stable|beta|dev>` instead.",
    );
    defaultRuntime.exit(1);
    return;
  }
  ...
  if (requestedChannel === "dev" && updateStatus.installKind !== "git") {
    ...
    if (!hasGit) {
      ...
      if (dirExists) {
        ...
        if (!empty) {
          defaultRuntime.error(
            `LOGICA_GIT_DIR points at a non-git directory: ${gitDir}. Set LOGICA_GIT_DIR to an empty folder or an logica checkout.`,
          );
          defaultRuntime.exit(1);
          return;
        }
      ...
      const ok = await confirm({
        message: stylePromptMessage(
          `Create a git checkout at ${gitDir}? (override via LOGICA_GIT_DIR)`,
        ),
        initialValue: true,
      });
      if (isCancel(ok) || !ok) {
...
export function registerUpdateCli(program: Command) {
  const update = program
    .command("update")
    .description("Update Logica to the latest version")
    ...
    .addHelpText("after", () => {
      const examples = [
        ["logica update", "Update a source checkout (git)"],
        ["logica update --channel beta", "Switch to beta channel (git + npm)"],
        ["logica update --channel dev", "Switch to dev channel (git + npm)"],
        ["logica update --tag beta", "One-off update to a dist-tag or version"],
        ["logica update --no-restart", "Update without restarting the service"],
        ["logica update --json", "Output result as JSON"],
        ["logica update --yes", "Non-interactive (accept downgrade prompts)"],
        ["logica update wizard", "Interactive update wizard"],
        ["logica --update", "Shorthand for logica update"],
      ] as const;
...
${theme.heading("Switch channels:")}
  ...
  - Run logica update status to see the active channel and source
...
${theme.muted("Docs:")} ${formatDocsLink("/cli/update", "docs.logica.ai/cli/update")}`;
    })
    .action(async (opts) => {
      try {
        await updateCommand({
  ...
  update
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["logica update status", "Show channel + version status."],
          ["logica update status --json", "JSON output."],
          ["logica update status --timeout 10", "Custom timeout."],
        ])}\n\n${theme.heading("Notes:")}\n${theme.muted(
          "- Shows current update channel (stable/beta/dev) and source",
        )}\n${theme.muted("- Includes git tag/branch/SHA for source checkouts")}\n\n${theme.muted(
          "Docs:",
        )} ${formatDocsLink("/cli/update", "docs.logica.ai/cli/update")}`,
    )
    .action(async (opts) => {
      try {
