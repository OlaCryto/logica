...
function summarizeGroupPolicy(cfg: LogicaConfig): {
  open: number;
  allowlist: number;
  other: number;
} {
...
function collectModels(cfg: LogicaConfig): ModelRef[] {
  const out: ModelRef[] = [];
...
function hasWebSearchKey(cfg: LogicaConfig, env: NodeJS.ProcessEnv): boolean {
...
function isWebSearchEnabled(cfg: LogicaConfig, env: NodeJS.ProcessEnv): boolean {
...
function isWebFetchEnabled(cfg: LogicaConfig): boolean {
...
function isBrowserEnabled(cfg: LogicaConfig): boolean {
  try {
...
function listGroupPolicyOpen(cfg: LogicaConfig): string[] {
  const out: string[] = [];
  const channels = cfg.channels as Record<string, unknown> | undefined;
  if (!channels || typeof channels !== "object") {
    return out;
...
export function collectAttackSurfaceSummaryFindings(cfg: LogicaConfig): SecurityAuditFinding[] {
...
export function collectSyncedFolderFindings(params: {
  ...
  if (isProbablySyncedPath(params.stateDir) || isProbablySyncedPath(params.configPath)) {
    findings.push({
      ...
      remediation: `Keep LOGICA_STATE_DIR on a local-only volume and re-run "${formatCliCommand("logica security audit --fix")}".`,
    });
  }
  return findings;
}

export function collectSecretsInConfigFindings(cfg: LogicaConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
  const password =
  ...
  if (password && !looksLikeEnvRef(password)) {
    findings.push({
      ...
      remediation:
        "Prefer LOGICA_GATEWAY_PASSWORD (env) and remove gateway.auth.password from disk.",
    });
  }
...
export function collectHooksHardeningFindings(cfg: LogicaConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
export function collectModelHygieneFindings(cfg: LogicaConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
export function collectExposureMatrixFindings(cfg: LogicaConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
