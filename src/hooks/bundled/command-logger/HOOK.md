...
homepage: https://docs.logica.ai/hooks#command-logger
metadata:
  {
    "logica":
      {
...
2. **Appends to log file** - Writes a JSON line to `~/.logica/logs/commands.log`
...
1. **Manual rotation**:
   ...
   ```bash
   mv ~/.logica/logs/commands.log ~/.logica/logs/commands.log.old
   ```

2. **Use logrotate** (Linux):
   Create `/etc/logrotate.d/logica`:
   ```
   /home/username/.logica/logs/commands.log {
       weekly
       rotate 4
       compress
       missingok
