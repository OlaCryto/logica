...
This directory contains hooks that ship with Logica. These hooks are automatically discovered and can be enabled/disabled via CLI or configuration.
...
**Docs**: https://docs.logica.ai/hooks/soul-evil
...
Hooks can be configured in `~/.logica/logica.json`:
...
2. Restart gateway: `pkill -9 -f 'logica.*gateway' && pnpm logica gateway`
3. Enable the hook: `logica hooks enable my-hook`
4. Trigger the event (e.g., send `/new` command)
5. Check gateway logs for hook execution

## Documentation

Full documentation: https://docs.logica.ai/hooks
