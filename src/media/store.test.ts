...
describe("media store", () => {
  ...
  const snapshotEnv = () => {
    for (const key of ["HOME", "USERPROFILE", "HOMEDRIVE", "HOMEPATH", "LOGICA_STATE_DIR"]) {
  ...
  beforeAll(async () => {
    snapshotEnv();
    home = await fs.mkdtemp(path.join(os.tmpdir(), "logica-test-home-"));
    process.env.HOME = home;
    process.env.USERPROFILE = home;
    process.env.LOGICA_STATE_DIR = path.join(home, ".logica");
    ...
    await fs.mkdir(path.join(home, ".logica"), { recursive: true });
  ...
  it("creates and returns media directory", async () => {
    await withTempStore(async (store, home) => {
      ...
      expect(path.normalize(dir)).toContain(`${path.sep}.logica${path.sep}media`);
