...
describe("browser config", () => {
  it("defaults to enabled with loopback defaults and lobster-orange color", () => {
    ...
    const logica = resolveProfile(resolved, "logica");
    expect(logica?.driver).toBe("logica");
    expect(logica?.cdpPort).toBe(18800);
    expect(logica?.cdpUrl).toBe("http://127.0.0.1:18800");
    expect(resolved.remoteCdpTimeoutMs).toBe(1500);
    expect(resolved.remoteCdpHandshakeTimeoutMs).toBe(3000);
  });

  it("derives default ports from LOGICA_GATEWAY_PORT when unset", () => {
    const prev = process.env.LOGICA_GATEWAY_PORT;
    process.env.LOGICA_GATEWAY_PORT = "19001";
    try {
      const resolved = resolveBrowserConfig(undefined);
      ...
      const logica = resolveProfile(resolved, "logica");
      expect(logica?.cdpPort).toBe(19012);
      expect(logica?.cdpUrl).toBe("http://127.0.0.1:19012");
    } finally {
      if (prev === undefined) {
        delete process.env.LOGICA_GATEWAY_PORT;
      } else {
        process.env.LOGICA_GATEWAY_PORT = prev;
      }
    }
  });
  ...
  it("derives default ports from gateway.port when env is unset", () => {
    ...
    try {
      ...
      const logica = resolveProfile(resolved, "logica");
      expect(logica?.cdpPort).toBe(19022);
      expect(logica?.cdpUrl).toBe("http://127.0.0.1:19022");
    } finally {
      if (prev === undefined) {
        delete process.env.LOGICA_GATEWAY_PORT;
      } else {
  ...
  it("treats non-loopback cdpUrl as remote", () => {
    ...
    const profile = resolveProfile(resolved, "logica");
  ...
  it("does not add the built-in chrome extension profile if the derived relay port is already used", () => {
    const resolved = resolveBrowserConfig({
      profiles: {
        logica: { cdpPort: 18792, color: "#FF4500" },
      },
    });
    expect(resolveProfile(resolved, "chrome")).toBe(null);
    expect(resolved.defaultProfile).toBe("logica");
  });
});
