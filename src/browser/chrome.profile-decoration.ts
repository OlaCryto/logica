...
function decoratedMarkerPath(userDataDir: string) {
  return path.join(userDataDir, ".logica-profile-decorated");
}
 ...
 */
export function decorateLogicaProfile(
  userDataDir: string,
  opts?: { name?: string; color?: string },
) {
  const desiredName = opts?.name ?? DEFAULT_LOGICA_BROWSER_PROFILE_NAME;
  const desiredColor = (opts?.color ?? DEFAULT_LOGICA_BROWSER_COLOR).toUpperCase();
