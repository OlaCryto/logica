  ...
  try {
    if (boxes.length > 0) {
      await page.evaluate((labels) => {
        const existing = document.querySelectorAll("[data-logica-labels]");
        existing.forEach((el) => el.remove());

        const root = document.createElement("div");
        root.setAttribute("data-logica-labels", "1");
        ...
        for (const label of labels) {
          ...
          box.setAttribute("data-logica-labels", "1");
          ...
          tag.setAttribute("data-logica-labels", "1");
