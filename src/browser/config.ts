import type { BrowserConfig, BrowserProfileConfig, LogicaConfig } from "../config/config.js";
...
import {
  ...
  DEFAULT_LOGICA_BROWSER_ENABLED,
...
export type ResolvedBrowserProfile = {
  ...
  cdpIsLoopback: boolean;
  color: string;
  driver: "logica" | "extension";
};

function normalizeHexColor(raw: string | undefined) {
  const value = (raw ?? "").trim();
  if (!value) {
    return DEFAULT_LOGICA_BROWSER_COLOR;
  }
...
/**
 * Ensure the default "logica" profile exists in the profiles map.
...
function ensureDefaultProfile(
  ...
  if (!result[DEFAULT_LOGICA_BROWSER_PROFILE_NAME]) {
    result[DEFAULT_LOGICA_BROWSER_PROFILE_NAME] = {
 ...
 *
 * Note: this is an Logica browser profile (routing config), not a Chrome user profile.
...
function ensureDefaultChromeExtensionProfile(
  ...
  // (legacy single-profile configs may use controlPort+1 for logica/logica CDP).
...
export function resolveBrowserConfig(
  ...
  rootConfig?: LogicaConfig,
): ResolvedBrowserConfig {
  const enabled = cfg?.enabled ?? DEFAULT_LOGICA_BROWSER_ENABLED;
...
export function resolveProfile(
  ...
  let cdpUrl = "";
  const driver = profile.driver === "extension" ? "extension" : "logica";
