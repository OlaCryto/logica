...
const execHostEnforced = process.env.LOGICA_NODE_EXEC_HOST?.trim().toLowerCase() === "app";
const execHostFallbackAllowed =
  process.env.LOGICA_NODE_EXEC_FALLBACK?.trim().toLowerCase() !== "0";
...
function ensureNodePathEnv(): string {
  ensureLogicaCliOnPath({ pathEnv: process.env.PATH ?? "" });
