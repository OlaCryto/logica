...
const ETC_LOGICA_DIR = path.join(ROOT_DIR, "etc", "logica");
const SHARED_DIR = path.join(ROOT_DIR, "shared");

const DEFAULT_BASE_PATH = path.join(CONFIG_DIR, "logica.json");

function configPath(...parts: string[]) {
  return path.join(CONFIG_DIR, ...parts);
}

function etcLogicaPath(...parts: string[]) {
  return path.join(ETC_LOGICA_DIR, ...parts);
}
...
describe("resolveConfigIncludes", () => {
  ...
  it("resolves absolute path $include", () => {
    const absolute = etcLogicaPath("agents.json");
  ...
  it("resolves parent directory references", () => {
    ...
    expect(resolve(obj, files, configPath("sub", "logica.json"))).toEqual({
      shared: true,
    });
  });
});
