...
import type { LogicaConfig, ConfigValidationIssue } from "./types.js";
...
import { LogicaSchema } from "./zod-schema.js";
...
function validateIdentityAvatar(config: LogicaConfig): ConfigValidationIssue[] {
...
export function validateConfigObject(
  raw: unknown,
): { ok: true; config: LogicaConfig } | { ok: false; issues: ConfigValidationIssue[] } {
  ...
  const validated = LogicaSchema.safeParse(raw);
  ...
  const duplicates = findDuplicateAgentDirs(validated.data as LogicaConfig);
  ...
  const avatarIssues = validateIdentityAvatar(validated.data as LogicaConfig);
  ...
  return {
    ...
    config: applyModelDefaults(
      applyAgentDefaults(applySessionDefaults(validated.data as LogicaConfig)),
    ),
  };
}
