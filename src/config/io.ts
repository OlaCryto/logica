...
import type { LogicaConfig, ConfigFileSnapshot, LegacyConfigIssue } from "./types.js";
...
import { compareLogicaVersions } from "./version.js";
...
function coerceConfig(value: unknown): LogicaConfig {
  if (!value || typeof value !== "object" || Array.isArray(value)) {
    return {};
  }
  return value as LogicaConfig;
}
...
function stampConfigVersion(cfg: LogicaConfig): LogicaConfig {
...
function warnIfConfigFromFuture(cfg: LogicaConfig, logger: Pick<typeof console, "warn">): void {
  const touched = cfg.meta?.lastTouchedVersion;
  if (!touched) {
    return;
  }
  const cmp = compareLogicaVersions(VERSION, touched);
  if (cmp === null) {
    return;
  }
  if (cmp < 0) {
    logger.warn(
      `Config was last written by a newer Logica (${touched}); current version is ${VERSION}.`,
    );
  }
}

function applyConfigEnv(cfg: LogicaConfig, env: NodeJS.ProcessEnv): void {
...
export function createConfigIO(overrides: ConfigIoDeps = {}) {
  ...
  function loadConfig(): LogicaConfig {
    try {
      ...
      if (resolved && typeof resolved === "object" && "env" in resolved) {
        applyConfigEnv(resolved as LogicaConfig, deps.env);
      }
      ...
      const preValidationDuplicates = findDuplicateAgentDirs(resolvedConfig as LogicaConfig, {
        env: deps.env,
  ...
  async function writeConfigFile(cfg: LogicaConfig) {
    clearConfigCache();
...
// module scope. `LOGICA_CONFIG_PATH` (and friends) are expected to work even
...
function resolveConfigCacheMs(env: NodeJS.ProcessEnv): number {
  const raw = env.LOGICA_CONFIG_CACHE_MS?.trim();
  if (raw === "" || raw === "0") {
    return 0;
  }
  if (!raw) {
...
function shouldUseConfigCache(env: NodeJS.ProcessEnv): boolean {
  if (env.LOGICA_DISABLE_CONFIG_CACHE?.trim()) {
    return false;
  }
...
export function loadConfig(): LogicaConfig {
...
export async function writeConfigFile(cfg: LogicaConfig): Promise<void> {
  clearConfigCache();
