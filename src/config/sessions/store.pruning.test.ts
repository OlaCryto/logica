...
// Mock loadConfig so resolveMaintenanceConfig() never reads a real logica.json.
...
describe("rotateSessionFile", () => {
  ...
  beforeEach(async () => {
    testDir = await fs.mkdtemp(path.join(os.tmpdir(), "logica-rotate-"));
...
describe("Integration: saveSessionStore with pruning", () => {
  ...
  beforeEach(async () => {
    testDir = await fs.mkdtemp(path.join(os.tmpdir(), "logica-pruning-integ-"));
    storePath = path.join(testDir, "sessions.json");
    savedCacheTtl = process.env.LOGICA_SESSION_CACHE_TTL_MS;
    process.env.LOGICA_SESSION_CACHE_TTL_MS = "0";
    clearSessionStoreCacheForTest();
  ...
  afterEach(async () => {
    ...
    if (savedCacheTtl === undefined) {
      delete process.env.LOGICA_SESSION_CACHE_TTL_MS;
    } else {
      process.env.LOGICA_SESSION_CACHE_TTL_MS = savedCacheTtl;
    }
  });
