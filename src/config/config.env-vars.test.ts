...
describe("config env vars", () => {
  ...
  it("loads ${VAR} substitutions from ~/.logica/.env on repeated runtime loads", async () => {
    await withTempHome(async (home) => {
      await withEnvOverride(
        {
          LOGICA_STATE_DIR: path.join(home, ".logica"),
          LOGICA_STATE_DIR: undefined,
          LOGICA_HOME: undefined,
          LOGICA_HOME: undefined,
          BRAVE_API_KEY: undefined,
          LOGICA_DISABLE_CONFIG_CACHE: "1",
        },
        async () => {
