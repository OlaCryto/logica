...
export function resolveConfigDir(
  ...
  const newDir = path.join(resolveRequiredHomeDir(env, homedir), ".logica");
  try {
...
function resolveHomeDisplayPrefix(): { home: string; prefix: string } | undefined {
  ...
  const explicitHome = process.env.LOGICA_HOME?.trim();
  if (explicitHome) {
    return { home, prefix: "$LOGICA_HOME" };
  }
  return { home, prefix: "~" };
}
...
// Configuration root; can be overridden via LOGICA_STATE_DIR.
