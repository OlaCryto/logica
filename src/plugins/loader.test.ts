...
import { loadLogicaPlugins } from "./loader.js";

type TempPlugin = { dir: string; file: string; id: string };

const tempDirs: string[] = [];
const prevBundledDir = process.env.LOGICA_BUNDLED_PLUGINS_DIR;
const EMPTY_PLUGIN_SCHEMA = { type: "object", additionalProperties: false, properties: {} };

function makeTempDir() {
  const dir = path.join(os.tmpdir(), `logica-plugin-${randomUUID()}`);
...
function writePlugin(params: {
  ...
  fs.writeFileSync(
    path.join(dir, "logica.plugin.json"),
    JSON.stringify(
      {
        id: params.id,
...
afterEach(() => {
  ...
  if (prevBundledDir === undefined) {
    ...
    process.env.LOGICA_BUNDLED_PLUGINS_DIR = prevBundledDir;
  }
});

describe("loadLogicaPlugins", () => {
  it("disables bundled plugins by default", () => {
    ...
    process.env.LOGICA_BUNDLED_PLUGINS_DIR = bundledDir;

    const registry = loadLogicaPlugins({
      cache: false,
      config: {
        plugins: {
          allow: ["bundled"],
    ...
    const enabledRegistry = loadLogicaPlugins({
      cache: false,
      config: {
        plugins: {
          allow: ["bundled"],
