...
import { discoverLogicaPlugins } from "./discovery.js";
...
function resolvePluginModuleExport(moduleExport: unknown): {
  definition?: LogicaPluginDefinition;
  register?: LogicaPluginDefinition["register"];
} {
  const resolved =
    moduleExport &&
  ...
  if (typeof resolved === "function") {
    return {
      register: resolved as LogicaPluginDefinition["register"],
    };
  }
  if (resolved && typeof resolved === "object") {
    const def = resolved as LogicaPluginDefinition;
...
export function loadLogicaPlugins(options: PluginLoadOptions = {}): PluginRegistry {
  ...
  const discovery = discoverLogicaPlugins({
    workspaceDir: options.workspaceDir,
  ...
  const jiti = createJiti(import.meta.url, {
    ...
    ...(pluginSdkAlias
      ? {
          alias: { "logica/plugin-sdk": pluginSdkAlias },
        }
      : {}),
  });
  ...
  for (const candidate of discovery.candidates) {
    ...
    let mod: LogicaPluginModule | null = null;
    try {
      mod = jiti(candidate.source) as LogicaPluginModule;
    } catch (err) {
