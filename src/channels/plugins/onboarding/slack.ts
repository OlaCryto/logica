...
function setSlackDmPolicy(cfg: LogicaConfig, dmPolicy: DmPolicy) {
  const allowFrom =
...
function buildSlackManifest(botName: string) {
  const safeName = botName.trim() || "Logica";
  const manifest = {
    display_information: {
      name: safeName,
      description: `${safeName} connector for Logica`,
    },
    features: {
      bot_user: {
...
function setSlackAllowFrom(cfg: LogicaConfig, allowFrom: string[]): LogicaConfig {
  return {
    ...cfg,
    channels: {
      ...cfg.channels,
