...
export function resolveHooksConfig(cfg: LogicaConfig): HooksConfigResolved | null {
...
function resolveKnownAgentIds(cfg: LogicaConfig, defaultAgentId: string): Set<string> {
...
export function extractHookToken(req: IncomingMessage): string | undefined {
  ...
  const headerToken =
    typeof req.headers["x-logica-token"] === "string"
      ? req.headers["x-logica-token"].trim()
      : "";
  if (headerToken) {
    return headerToken;
  }
