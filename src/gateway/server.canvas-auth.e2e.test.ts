...
async function withTempConfig(params: { cfg: unknown; run: () => Promise<void> }): Promise<void> {
  const prevConfigPath = process.env.LOGICA_CONFIG_PATH;
  const prevDisableCache = process.env.LOGICA_DISABLE_CONFIG_CACHE;

  const dir = await mkdtemp(path.join(os.tmpdir(), "logica-canvas-auth-test-"));
  const configPath = path.join(dir, "logica.json");
  ...
  try {
    ...
    if (prevConfigPath === undefined) {
      ...
      process.env.LOGICA_CONFIG_PATH = prevConfigPath;
    }
    if (prevDisableCache === undefined) {
      delete process.env.LOGICA_DISABLE_CONFIG_CACHE;
    } else {
      process.env.LOGICA_DISABLE_CONFIG_CACHE = prevDisableCache;
    }
