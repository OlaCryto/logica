...
describe("gateway hot reload", () => {
  ...
  beforeEach(() => {
    prevSkipChannels = process.env.LOGICA_SKIP_CHANNELS;
    prevSkipGmail = process.env.LOGICA_SKIP_GMAIL_WATCHER;
    process.env.LOGICA_SKIP_CHANNELS = "0";
  ...
  afterEach(() => {
    if (prevSkipChannels === undefined) {
      ...
      process.env.LOGICA_SKIP_CHANNELS = prevSkipChannels;
    }
    if (prevSkipGmail === undefined) {
      delete process.env.LOGICA_SKIP_GMAIL_WATCHER;
    } else {
      process.env.LOGICA_SKIP_GMAIL_WATCHER = prevSkipGmail;
    }
  });
