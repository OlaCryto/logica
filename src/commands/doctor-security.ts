...
import type { LogicaConfig, GatewayBindMode } from "../config/config.js";
...
export async function noteSecurityWarnings(cfg: LogicaConfig) {
  const warnings: string[] = [];
  const auditHint = `- Run: ${formatCliCommand("logica security audit --deep")}`;
    ...
    if (!hasSharedSecret) {
      const authFixLines =
        resolvedAuth.mode === "password"
          ? [
              `  Fix: ${formatCliCommand("logica configure")} to set a password`,
              `  Or switch to token: ${formatCliCommand("logica config set gateway.auth.mode token")}`,
            ]
          : [
              `  Fix: ${formatCliCommand("logica doctor --fix")} to generate a token`,
      ...
      warnings.push(
        ...
        `  Fix: ${formatCliCommand("logica config set gateway.bind loopback")}`,
        ...authFixLines,
      );
    } else {
