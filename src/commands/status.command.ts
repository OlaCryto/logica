...
export async function statusCommand(
  ...
  runtime.log(theme.heading("Logica status"));
  runtime.log("");
  ...
  runtime.log(theme.muted(`Full report: ${formatCliCommand("logica security audit")}`));
  runtime.log(theme.muted(`Deep probe: ${formatCliCommand("logica security audit --deep")}`));
  ...
  runtime.log("");
  runtime.log("FAQ: https://docs.logica.ai/faq");
  runtime.log("Troubleshooting: https://docs.logica.ai/troubleshooting");
  runtime.log("");
  ...
  runtime.log(`  Need to share?      ${formatCliCommand("logica status --all")}`);
  runtime.log(`  Need to debug live? ${formatCliCommand("logica logs --follow")}`);
  if (gatewayReachable) {
    runtime.log(`  Need to test channels? ${formatCliCommand("logica status --deep")}`);
  } else {
    runtime.log(`  Fix reachability first: ${formatCliCommand("logica gateway probe")}`);
  }
}
