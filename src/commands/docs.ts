...
const SEARCH_TOOL = "https://docs.logica.ai/mcp.SearchLogica";
...
export async function docsSearchCommand(queryParts: string[], runtime: RuntimeEnv) {
  ...
  if (!query) {
    const docs = formatDocsLink("/", "docs.logica.ai");
    if (isRich()) {
      runtime.log(`${theme.muted("Docs:")} ${docs}`);
      runtime.log(`${theme.muted("Search:")} ${formatCliCommand('logica docs "your query"')}`);
    } else {
      runtime.log("Docs: https://docs.logica.ai/");
      runtime.log(`Search: ${formatCliCommand('logica docs "your query"')}`);
    }
    return;
  }
