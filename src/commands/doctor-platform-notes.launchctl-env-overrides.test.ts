...
describe("noteMacLaunchctlGatewayEnvOverrides", () => {
  it("prints clear unsetenv instructions for token override", async () => {
    ...
    const getenv = vi.fn(async (name: string) =>
      name === "LOGICA_GATEWAY_TOKEN" ? "launchctl-token" : undefined,
    );
    const cfg = {
      gateway: {
        auth: {
    ...
    expect(message).toContain("LOGICA_GATEWAY_TOKEN");
    expect(message).toContain("launchctl unsetenv LOGICA_GATEWAY_TOKEN");
    expect(message).not.toContain("LOGICA_GATEWAY_PASSWORD");
  });
