...
beforeEach(() => {
  ...
  resolveLogicaPackageRoot.mockReset().mockResolvedValue(null);
  ...
  setStdinTty(true);
  originalStateDir = process.env.LOGICA_STATE_DIR;
  originalUpdateInProgress = process.env.LOGICA_UPDATE_IN_PROGRESS;
  process.env.LOGICA_UPDATE_IN_PROGRESS = "1";
  tempStateDir = fs.mkdtempSync(path.join(os.tmpdir(), "logica-doctor-state-"));
...
afterEach(() => {
  ...
  if (originalStateDir === undefined) {
    ...
    process.env.LOGICA_STATE_DIR = originalStateDir;
  }
  if (originalUpdateInProgress === undefined) {
    delete process.env.LOGICA_UPDATE_IN_PROGRESS;
  } else {
    process.env.LOGICA_UPDATE_IN_PROGRESS = originalUpdateInProgress;
  }
  if (tempStateDir) {
...
const resolveLogicaPackageRoot = vi.fn().mockResolvedValue(null);
...
vi.mock("../plugins/loader.js", () => ({
  loadLogicaPlugins: () => ({ plugins: [], diagnostics: [] }),
}));
...
vi.mock("../infra/logica-root.js", () => ({
  resolveLogicaPackageRoot,
}));
...
describe("doctor command", () => {
  it("warns when the state directory is missing", async () => {
    ...
    const missingDir = fs.mkdtempSync(path.join(os.tmpdir(), "logica-missing-state-"));
    fs.rmSync(missingDir, { recursive: true, force: true });
    process.env.LOGICA_STATE_DIR = missingDir;
    note.mockClear();
  ...
  it("skips gateway auth warning when LOGICA_GATEWAY_TOKEN is set", async () => {
    readConfigFileSnapshot.mockResolvedValue({
      path: "/tmp/logica.json",
      exists: true,
      raw: "{}",
    ...
    process.env.LOGICA_GATEWAY_TOKEN = "env-token-1234567890";
    note.mockClear();
