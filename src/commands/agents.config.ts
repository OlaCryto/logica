...
export function listAgentEntries(cfg: LogicaConfig): AgentEntry[] {
...
function resolveAgentName(cfg: LogicaConfig, agentId: string) {
...
function resolveAgentModel(cfg: LogicaConfig, agentId: string) {
  const entry = listAgentEntries(cfg).find(
    (agent) => normalizeAgentId(agent.id) === normalizeAgentId(agentId),
  );
  if (entry?.model) {
...
export function buildAgentSummaries(cfg: LogicaConfig): AgentSummary[] {
