...
const mocks = vi.hoisted(() => {
  ...
  return {
    store,
    resolveLogicaAgentDir: vi.fn().mockReturnValue("/tmp/logica-agent"),
    resolveAgentDir: vi.fn().mockReturnValue("/tmp/logica-agent"),
    ...
    resolveAuthStorePathForDisplay: vi
      .fn()
      .mockReturnValue("/tmp/logica-agent/auth-profiles.json"),
...
vi.mock("../../agents/agent-paths.js", () => ({
  resolveLogicaAgentDir: mocks.resolveLogicaAgentDir,
}));
...
describe("modelsStatusCommand auth overview", () => {
  it("includes masked auth sources in JSON output", async () => {
    ...
    expect(mocks.resolveLogicaAgentDir).toHaveBeenCalled();
    expect(payload.defaultModel).toBe("anthropic/claude-opus-4-5");
    expect(payload.auth.storePath).toBe("/tmp/logica-agent/auth-profiles.json");
  ...
  it("uses agent overrides and reports sources", async () => {
    ...
    mocks.resolveAgentDir.mockReturnValue("/tmp/logica-agent-custom");
    ...
    try {
      ...
      expect(payload.agentDir).toBe("/tmp/logica-agent-custom");
