...
export async function updateConfig(
  mutator: (cfg: LogicaConfig) => LogicaConfig,
...
export function resolveModelTarget(params: { raw: string; cfg: LogicaConfig }): {
  provider: string;
  model: string;
} {
...
export function buildAllowlistSet(cfg: LogicaConfig): Set<string> {
...
export function resolveKnownAgentId(params: {
  ...
  if (!knownAgents.includes(agentId)) {
    throw new Error(
      `Unknown agent id "${raw}". Use "${formatCliCommand("logica agents list")}" to see configured agents.`,
    );
  }
  return agentId;
}
