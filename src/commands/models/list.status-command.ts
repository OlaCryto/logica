...
export async function modelsStatusCommand(
  ...
  const agentDir = agentId ? resolveAgentDir(cfg, agentId) : resolveLogicaAgentDir();
  ...
  if (missingProvidersInUse.length > 0) {
    ...
    for (const provider of missingProvidersInUse) {
      const hint =
        provider === "anthropic"
          ? `Run \`claude setup-token\`, then \`${formatCliCommand("logica models auth setup-token")}\` or \`${formatCliCommand("logica configure")}\`.`
          : `Run \`${formatCliCommand("logica configure")}\` or set an API key env var.`;
