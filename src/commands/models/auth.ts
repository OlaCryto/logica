...
import { readConfigFileSnapshot, type LogicaConfig } from "../../config/config.js";
...
function applyDefaultModel(cfg: LogicaConfig, model: string): LogicaConfig {
...
export async function modelsAuthLoginCommand(opts: LoginOptions, runtime: RuntimeEnv) {
  ...
  if (providers.length === 0) {
    throw new Error(
      `No provider plugins found. Install one via \`${formatCliCommand("logica plugins install")}\`.`,
    );
  }
