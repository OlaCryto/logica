...
describe("onboard (non-interactive): gateway and remote auth", () => {
  ...
  beforeAll(async () => {
    ...
    tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "logica-onboard-"));
  ...
  afterAll(async () => {
    ...
    process.env.LOGICA_GATEWAY_PASSWORD = prev.password;
  });
  ...
  it("writes gateway token auth into config and gateway enforces it", async () => {
    ...
    const workspace = path.join(stateDir, "logica");
  ...
  it("auto-generates token auth when binding LAN and persists the token", async () => {
    ...
    process.env.LOGICA_STATE_DIR = stateDir;
    process.env.LOGICA_CONFIG_PATH = path.join(stateDir, "logica.json");
