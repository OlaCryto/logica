...
function resolveMode(cfg: LogicaConfig): "local" | "remote" {
...
export async function doctorCommand(
  ...
  const root = await resolveLogicaPackageRoot({
  ...
  let cfg: LogicaConfig = configResult.cfg;

  const configPath = configResult.path ?? CONFIG_PATH;
  if (!cfg.gateway?.mode) {
    const lines = [
      "gateway.mode is unset; gateway start will be blocked.",
      `Fix: run ${formatCliCommand("logica configure")} and set Gateway mode (local/remote).`,
      `Or set directly: ${formatCliCommand("logica config set gateway.mode local")}`,
    ];
    if (!fs.existsSync(configPath)) {
      lines.push(`Missing config: run ${formatCliCommand("logica setup")} first.`);
    }
    note(lines.join("\n"), "Gateway");
  }
  ...
  if (shouldWriteConfig) {
    ...
    runtime.log(`Run "${formatCliCommand("logica doctor --fix")}" to apply changes.`);
  }
