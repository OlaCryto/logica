...
function captureEnv(): EnvSnapshot {
  return {
    ...
    password: process.env.LOGICA_GATEWAY_PASSWORD,
    disableConfigCache: process.env.LOGICA_DISABLE_CONFIG_CACHE,
  };
}
...
function restoreEnv(prev: EnvSnapshot): void {
  ...
  restoreEnvVar("LOGICA_STATE_DIR", prev.stateDir);
  restoreEnvVar("LOGICA_CONFIG_PATH", prev.configPath);
  restoreEnvVar("LOGICA_SKIP_CHANNELS", prev.skipChannels);
  restoreEnvVar("LOGICA_SKIP_GMAIL_WATCHER", prev.skipGmail);
  restoreEnvVar("LOGICA_SKIP_CRON", prev.skipCron);
  restoreEnvVar("LOGICA_SKIP_CANVAS_HOST", prev.skipCanvas);
  restoreEnvVar("LOGICA_GATEWAY_TOKEN", prev.token);
  restoreEnvVar("LOGICA_GATEWAY_PASSWORD", prev.password);
  restoreEnvVar("LOGICA_DISABLE_CONFIG_CACHE", prev.disableConfigCache);
}
...
async function withOnboardEnv(
  ...
  process.env.LOGICA_DISABLE_CONFIG_CACHE = "1";
  ...
  const configPath = path.join(tempHome, "logica.json");
  process.env.HOME = tempHome;
  process.env.LOGICA_STATE_DIR = tempHome;
  process.env.LOGICA_CONFIG_PATH = configPath;
  vi.resetModules();
...
describe("onboard (non-interactive): provider auth", () => {
  it("stores xAI API key and sets default model", async () => {
    await withOnboardEnv("logica-onboard-xai-", async ({ configPath, runtime }) => {
  ...
  it("stores Vercel AI Gateway API key and sets default model", async () => {
    await withOnboardEnv("logica-onboard-ai-gateway-", async ({ configPath, runtime }) => {
      await runNonInteractive(
        {
          nonInteractive: true,
  ...
  it("stores token auth profile", async () => {
    await withOnboardEnv("logica-onboard-token-", async ({ configPath, runtime }) => {
  ...
  it("stores OpenAI API key and sets OpenAI default model", async () => {
    await withOnboardEnv("logica-onboard-openai-", async ({ configPath, runtime }) => {
      await runNonInteractive(
        {
          nonInteractive: true,
  ...
  it("stores Cloudflare AI Gateway API key and metadata", async () => {
    await withOnboardEnv("logica-onboard-cf-gateway-", async ({ configPath, runtime }) => {
      await runNonInteractive(
        {
          nonInteractive: true,
  ...
  it("infers Cloudflare auth choice from API key flags", async () => {
    await withOnboardEnv("logica-onboard-cf-gateway-infer-", async ({ configPath, runtime }) => {
      await runNonInteractive(
        {
          nonInteractive: true,
          cloudflareAiGatewayAccountId: "cf-account-id",
