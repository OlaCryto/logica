...
beforeAll(() => {
  previousProfile = process.env.LOGICA_PROFILE;
  process.env.LOGICA_PROFILE = "isolated";
});
...
afterAll(() => {
  if (previousProfile === undefined) {
    ...
    process.env.LOGICA_PROFILE = previousProfile;
  }
});
...
vi.mock("../infra/logica-root.js", () => ({
  resolveLogicaPackageRoot: vi.fn().mockResolvedValue("/tmp/logica"),
}));
...
vi.mock("../infra/update-check.js", () => ({
  checkUpdateStatus: vi.fn().mockResolvedValue({
    ...
    deps: {
      manager: "pnpm",
      status: "ok",
      lockfilePath: "/tmp/logica/pnpm-lock.yaml",
      markerPath: "/tmp/logica/node_modules/.modules.yaml",
    },
...
describe("statusCommand", () => {
  ...
  it("prints formatted lines otherwise", async () => {
    ...
    expect(logs.some((l) => l.includes("Logica status"))).toBe(true);
    ...
    expect(
      logs.some(
        (l) =>
          l.includes("logica status --all") ||
          l.includes("logica --profile isolated status --all") ||
          l.includes("logica status --all") ||
          l.includes("logica --profile isolated status --all"),
      ),
    ).toBe(true);
  ...
  it("shows gateway auth when reachable", async () => {
    ...
    process.env.LOGICA_GATEWAY_TOKEN = "abcd1234";
    try {
