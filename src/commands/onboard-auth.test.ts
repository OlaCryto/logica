...
const requireAgentDir = () => {
  const agentDir = process.env.LOGICA_AGENT_DIR;
  if (!agentDir) {
    throw new Error("LOGICA_AGENT_DIR not set");
  }
  return agentDir;
};

describe("writeOAuthCredentials", () => {
  const previousStateDir = process.env.LOGICA_STATE_DIR;
  const previousAgentDir = process.env.LOGICA_AGENT_DIR;
  ...
  afterEach(async () => {
    ...
    if (previousAgentDir === undefined) {
      delete process.env.LOGICA_AGENT_DIR;
    } else {
      process.env.LOGICA_AGENT_DIR = previousAgentDir;
    }
    ...
    delete process.env.LOGICA_OAUTH_DIR;
  });

  it("writes auth-profiles.json under LOGICA_AGENT_DIR when set", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "logica-oauth-"));
    process.env.LOGICA_STATE_DIR = tempStateDir;
    process.env.LOGICA_AGENT_DIR = path.join(tempStateDir, "agent");
    process.env.PI_CODING_AGENT_DIR = process.env.LOGICA_AGENT_DIR;
...
describe("setMinimaxApiKey", () => {
  ...
  it("writes to LOGICA_AGENT_DIR when set", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "logica-minimax-"));
    process.env.LOGICA_STATE_DIR = tempStateDir;
    process.env.LOGICA_AGENT_DIR = path.join(tempStateDir, "custom-agent");
    process.env.PI_CODING_AGENT_DIR = process.env.LOGICA_AGENT_DIR;
