...
describe("ensureDir", () => {
  it("creates nested directory", async () => {
    const tmp = await fs.promises.mkdtemp(path.join(os.tmpdir(), "logica-test-"));
...
describe("jidToE164", () => {
  ...
  it("maps @lid from authDir mapping files", () => {
    const authDir = fs.mkdtempSync(path.join(os.tmpdir(), "logica-auth-"));
  ...
  it("falls back through lidMappingDirs in order", () => {
    const first = fs.mkdtempSync(path.join(os.tmpdir(), "logica-lid-a-"));
    const second = fs.mkdtempSync(path.join(os.tmpdir(), "logica-lid-b-"));
...
describe("resolveConfigDir", () => {
  it("prefers ~/.logica when legacy dir is missing", async () => {
    const root = await fs.promises.mkdtemp(path.join(os.tmpdir(), "logica-config-dir-"));
    try {
...
describe("resolveHomeDir", () => {
  it("prefers LOGICA_HOME over HOME", () => {
    vi.stubEnv("LOGICA_HOME", "/srv/logica-home");
    vi.stubEnv("HOME", "/home/other");

    expect(resolveHomeDir()).toBe(path.resolve("/srv/logica-home"));
...
describe("shortenHomePath", () => {
  it("uses $LOGICA_HOME prefix when LOGICA_HOME is set", () => {
    vi.stubEnv("LOGICA_HOME", "/srv/logica-home");
    vi.stubEnv("HOME", "/home/other");

    expect(shortenHomePath(`${path.resolve("/srv/logica-home")}/.logica/logica.json`)).toBe(
      "$LOGICA_HOME/.logica/logica.json",
    );
...
describe("shortenHomeInString", () => {
  it("uses $LOGICA_HOME replacement when LOGICA_HOME is set", () => {
    vi.stubEnv("LOGICA_HOME", "/srv/logica-home");
    vi.stubEnv("HOME", "/home/other");

    expect(
      shortenHomeInString(`config: ${path.resolve("/srv/logica-home")}/.logica/logica.json`),
    ).toBe("config: $LOGICA_HOME/.logica/logica.json");
...
describe("resolveUserPath", () => {
  ...
  it("expands ~/ to home dir", () => {
    expect(resolveUserPath("~/logica")).toBe(path.resolve(os.homedir(), "logica"));
  });
  ...
  it("prefers LOGICA_HOME for tilde expansion", () => {
    vi.stubEnv("LOGICA_HOME", "/srv/logica-home");
    vi.stubEnv("HOME", "/home/other");

    expect(resolveUserPath("~/logica")).toBe(path.resolve("/srv/logica-home", "logica"));
