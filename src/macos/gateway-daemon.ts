...
declare const __LOGICA_VERSION__: string | undefined;

const BUNDLED_VERSION =
  (typeof __LOGICA_VERSION__ === "string" && __LOGICA_VERSION__) ||
  process.env.LOGICA_BUNDLED_VERSION ||
  "0.0.0";
...
async function main() {
  if (hasFlag(args, "--version") || hasFlag(args, "-v")) {
    // Match `logica --version` behavior for Swift env/version checks.
  ...
  const portRaw =
    argValue(args, "--port") ??
    process.env.LOGICA_GATEWAY_PORT ??
    process.env.LOGICA_GATEWAY_PORT ??
  ...
  const bindRaw =
    argValue(args, "--bind") ??
    process.env.LOGICA_GATEWAY_BIND ??
    process.env.LOGICA_GATEWAY_BIND ??
    cfg.gateway?.bind ??
    "loopback";
  const bind =
