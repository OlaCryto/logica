...
describe("canvas host", () => {
  it("injects live reload script", () => {
    ...
    expect(out).toContain("logicaCanvasA2UIAction");
    expect(out).toContain("logicaSendUserAction");
  });

  it("creates a default index.html when missing", async () => {
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), "logica-canvas-"));
    ...
    try {
      ...
      expect(html).toContain("logicaSendUserAction");
  ...
  it("serves the gateway-hosted A2UI scaffold", async () => {
    ...
    try {
      ...
      await fs.writeFile(bundlePath, "window.logicaA2UI = {};", "utf8");
      createdBundle = true;
    }
    ...
    try {
      const res = await fetch(`http://127.0.0.1:${server.port}/__logica__/a2ui/`);
      const html = await res.text();
      expect(res.status).toBe(200);
      expect(html).toContain("logica-a2ui-host");
      expect(html).toContain("logicaCanvasA2UIAction");

      const bundleRes = await fetch(
        `http://127.0.0.1:${server.port}/__logica__/a2ui/a2ui.bundle.js`,
      );
      const js = await bundleRes.text();
      expect(bundleRes.status).toBe(200);
      expect(js).toContain("logicaA2UI");
    } finally {
      await server.close();
      if (createdBundle) {
