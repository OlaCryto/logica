...
describe("canvas host state dir defaults", () => {
  ...
  it("uses LOGICA_STATE_DIR for the default canvas root", async () => {
    const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "logica-canvas-state-"));
    ...
    try {
      ...
      expect(indexContents).toContain("Logica Canvas");
    } finally {
      await handler.close();
