...
describe("setupOnboardingShellCompletion", () => {
  it("QuickStart: installs without prompting", async () => {
    ...
    const deps = {
      resolveCliName: () => "logica",
      checkShellCompletionStatus: vi.fn(async () => ({
        shell: "zsh",
        profileInstalled: false,
        cacheExists: false,
        cachePath: "/tmp/logica.zsh",
    ...
    expect(deps.ensureCompletionCacheExists).toHaveBeenCalledWith("logica");
    expect(deps.installCompletion).toHaveBeenCalledWith("zsh", true, "logica");
