...
async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  return withTempHomeBase(fn, { prefix: "logica-cron-" });
}

async function writeSessionStore(home: string) {
  const dir = path.join(home, ".logica", "sessions");
...
function makeCfg(
  home: string,
  storePath: string,
  overrides: Partial<LogicaConfig> = {},
): LogicaConfig {
  const base: LogicaConfig = {
    agents: {
      defaults: {
        model: "anthropic/claude-opus-4-5",
        workspace: path.join(home, "logica"),
      },
    },
...
describe("runCronIsolatedAgentTurn", () => {
  ...
  it("uses agentId for workspace, session key, and store paths", async () => {
    await withTempHome(async (home) => {
      ...
      const cfg = makeCfg(
        home,
        path.join(home, ".logica", "agents", "{agentId}", "sessions", "sessions.json"),
        {
          agents: {
