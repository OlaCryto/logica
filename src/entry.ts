...
function ensureExperimentalWarningSuppressed(): boolean {
  if (isTruthyEnvValue(process.env.LOGICA_NO_RESPAWN)) {
    return false;
  }
  if (isTruthyEnvValue(process.env.LOGICA_NODE_OPTIONS_READY)) {
    return false;
  }
  ...
  process.env.LOGICA_NODE_OPTIONS_READY = "1";
...
if (!ensureExperimentalWarningSuppressed()) {
  ...
  if (!parsed.ok) {
    ...
    console.error(`[logica] ${parsed.error}`);
    process.exit(2);
  }
