...
async function createDockerSetupSandbox(): Promise<DockerSetupSandbox> {
  const rootDir = await mkdtemp(join(tmpdir(), "logica-docker-setup-"));
...
function createEnv(
  ...
  return {
    ...
    LOGICA_GATEWAY_TOKEN: "test-token",
    LOGICA_CONFIG_DIR: join(sandbox.rootDir, "config"),
    LOGICA_WORKSPACE_DIR: join(sandbox.rootDir, "logica"),
    ...overrides,
  };
}
...
describe("docker-setup.sh", () => {
  it("handles unset optional env vars under strict mode", async () => {
    ...
    const env = createEnv(sandbox, {
      LOGICA_DOCKER_APT_PACKAGES: undefined,
      LOGICA_EXTRA_MOUNTS: undefined,
      LOGICA_HOME_VOLUME: undefined,
    });
    ...
    expect(envFile).toContain("LOGICA_DOCKER_APT_PACKAGES=");
    expect(envFile).toContain("LOGICA_EXTRA_MOUNTS=");
    expect(envFile).toContain("LOGICA_HOME_VOLUME=");
  });
  ...
  it("supports a home volume when extra mounts are empty", async () => {
    ...
    const env = createEnv(sandbox, {
      LOGICA_EXTRA_MOUNTS: "",
      LOGICA_HOME_VOLUME: "logica-home",
    });
    ...
    expect(extraCompose).toContain("logica-home:/home/node");
    expect(extraCompose).toContain("volumes:");
    expect(extraCompose).toContain("logica-home:");
  });
  ...
  it("plumbs LOGICA_DOCKER_APT_PACKAGES into .env and docker build args", async () => {
    const sandbox = await createDockerSetupSandbox();
    const env = createEnv(sandbox, {
      LOGICA_DOCKER_APT_PACKAGES: "ffmpeg build-essential",
      LOGICA_EXTRA_MOUNTS: "",
      LOGICA_HOME_VOLUME: "",
    });
    ...
    const envFile = await readFile(join(sandbox.rootDir, ".env"), "utf8");
    expect(envFile).toContain("LOGICA_DOCKER_APT_PACKAGES=ffmpeg build-essential");

    const log = await readFile(sandbox.logPath, "utf8");
    expect(log).toContain("--build-arg LOGICA_DOCKER_APT_PACKAGES=ffmpeg build-essential");
  });
